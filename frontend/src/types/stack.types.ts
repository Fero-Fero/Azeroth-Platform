...
export enum BuildPhase {
  ...
  CheckingModules = 'CheckingModules',
  ModuleCheckPassed = 'ModuleCheckPassed',
  Building = 'Building',
...
export interface StackConfigurationDto {
  ...
  moduleIds: string[]
  /** Per-stack git branch override keyed by catalog module id. Catalog branch is used when omitted. */
  moduleBranches?: Record<string, string>
  addonIds?: string[]
...
export interface ModuleCheckItemDto {
  moduleId: string
  name: string
  status: 'pending' | 'compiling' | 'passed' | 'failed' | 'skipped' | string
  error?: string | null
  commitSha?: string | null
  branch?: string | null
}

export interface ModuleCheckStatusDto {
  passed: boolean
  skipped?: boolean
  fingerprint?: string | null
  completedAt?: string | null
  items: ModuleCheckItemDto[]
}

export interface SyncStackModuleItemDto {
  moduleId: string
  name: string
  ok: boolean
  message: string
...
export interface SyncStackModulesResultDto {
  items: SyncStackModuleItemDto[]
}
...
export interface StackDetailsDto {
  ...
  expressProvisionPhase?: ExpressProvisionPhase
  /** True after Express created the game account admin / admin (GM 3). */
  expressGameAccountCreated?: boolean
  /** One-time Overview notice: all ready, press Start. */
  expressReadyNoticePending?: boolean
  /** Last per-module compile gate (null if a check has not been run). */
  moduleCheck?: ModuleCheckStatusDto | null
}

export type ExpressProvisionStatus = 'None' | 'Pending' | 'Running' | 'Completed' | 'Failed' | 'WaitingForClient'

export type ExpressProvisionPhase =
  | 'None'
  | 'SaveChoices'
  | 'DisableBots'
  | 'StartStack'
...
// Module DTO
export interface ModuleCompileProfile {
  checkoutFolder?: string | null
...
export interface ModuleDto {
  ...
  requiredModuleIds?: string[]
  // Compile/checkout recipe from the module install hook (conflicts, extra packages, folder alias).
  compile?: ModuleCompileProfile
}
