...
/**
 * Playerbots disable → start → Server Wide Progression (from the IP module) → re-enable.
 * Independent module steps (AH bot, dungeon sim, …) stay parallel.
 */
  ...
  return [
    { ...disablePlayerbotsStep(), sequenced: true },
    {
      ...startStackStep({
        ...
        when: playerbotsDisabledPhase,
        isComplete: (ctx) =>
          ctx.status.progress.getPlayerbotsPhase() === 'awaiting-reenable' ||
          ctx.status.progress.isPlayerbotsSetupComplete(),
        onStarted: (ctx) => ctx.status.progress.setPlayerbotsPhase('awaiting-reenable'),
      }),
      sequenced: true,
    ...
    ...(prepare ? [{ ...prepare, sequenced: true }] : []),
    {
      ...reenablePlayerbotsStep(),
      sequenced: true,
      dependsOn: [STEP_IDS.prepareProgression],
      progressApplies: (ctx) =>
        hasPlayerbotsModule(ctx.stack) && !isPlayerbotsSetupComplete(ctx.status),
    },
    ...independent,
