...
interface FileBrowserProps {
  ...
  /** Uploads a single file into a folder (relative dir), overwriting any file already there. */
  useUpload: (
    stackId: string
  ) => UseMutationResult<
    unknown,
    unknown,
    { dir: string; file: File; onProgress?: (percent: number) => void }
  >
...
export default function FileBrowser({
  ...
  const uploadInto = async (dir: string, files: File[]) => {
    ...
    setUploadError(null)
    setUploading({ dir, total: files.length, done: 0, name: files[0].name, percent: 0 })
    try {
      for (let i = 0; i < files.length; i++) {
        await uploadFile.mutateAsync({
          ...
          file: files[i],
          onProgress: (percent) =>
            setUploading({ dir, total: files.length, done: i, name: files[i].name, percent }),
        })
        setUploading({ dir, total: files.length, done: i + 1, name: files[i].name, percent: 100 })
      }
      ...
      {uploading && (
        <div className="mb-3 rounded-md bg-blue-50 px-3 py-2 text-sm text-blue-700">
          <div className="flex items-center gap-2">
            ...
            Uploading {Math.min(uploading.done + 1, uploading.total)}/{uploading.total} to{' '}
            <span className="font-mono">{uploading.dir || rootLabel}</span>
            <span className="truncate font-mono text-xs text-blue-600">{uploading.name}</span>
            <span className="ml-auto tabular-nums">{uploading.percent}%</span>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {!readOnly && (
                        entry.isLocked ? (
                        <span
                          title={entry.lockReason || `${entry.name} is locked and cannot be deleted here`}
