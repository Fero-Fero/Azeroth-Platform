...
/** Full-width row for values that do not fit the right-aligned single-line layout of ReviewRow. */
function ReviewBlock({ label, children }: { label: ReactNode; children: ReactNode }) {
  return (
    <div className="border-b border-gray-100 py-2 last:border-0">
      <dt className="mb-2 text-sm text-gray-500">{label}</dt>
      <dd>{children}</dd>
    </div>
  )
}
...
export function ReviewStep({
  ...
  const { data: catalogModules } = useModules(values.serverType)
  const selectedServerType = serverTypes?.find((type) => type.id === values.serverType)
  // Community modules are added by id and are not in the curated catalog, so fall back to the id.
  const selectedModuleNames = values.moduleIds
    .map((id: string) => catalogModules?.find((module) => module.id === id)?.name ?? id)
    ...
    <div className="space-y-5">
      ...
      <Section title="Modules">
        {selectedModuleNames.length > 0 ? (
          <ReviewBlock
            label={`Selected Modules (${selectedModuleNames.length})`}
          >
            <ul className="flex flex-wrap gap-1.5" aria-label="Selected modules">
              {selectedModuleNames.map((name: string) => (
                <li
                  key={name}
          ...
          <ReviewRow label="Selected Modules" value="None" />
        )}
        {values.serverType === ServerType.Express && (
          <ReviewRow
