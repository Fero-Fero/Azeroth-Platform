...
export type PlayerbotsSetupPhase = 'awaiting-start' | 'starting' | 'awaiting-reenable'

export type IpPostSyncGuide = 'validate' | 'apply' | 'return' | 'done'
...
type PersistedProgress = {
  ...
  ipPostSyncGuide: IpPostSyncGuide | null
}
...
export type SetupProgressStore = {
  ...
  getIpPostSyncGuide: () => IpPostSyncGuide | null
  setIpPostSyncGuide: (guide: IpPostSyncGuide | null) => void
...
function readJson(stackId: string): PersistedProgress | null {
  try {
    ...
    return {
      ...
      playerbotsPhase:
        parsed.playerbotsPhase === 'awaiting-start' ||
        parsed.playerbotsPhase === 'starting' ||
        parsed.playerbotsPhase === 'awaiting-reenable'
          ? parsed.playerbotsPhase
          : null,
      playerbotsSetupComplete: parsed.playerbotsSetupComplete === true,
      ipPostSyncGuide:
        parsed.ipPostSyncGuide === 'validate' ||
        parsed.ipPostSyncGuide === 'apply' ||
        parsed.ipPostSyncGuide === 'return' ||
        parsed.ipPostSyncGuide === 'done'
          ? parsed.ipPostSyncGuide
          : null,
    }
  } catch {
    return null
...
function migrateLegacy(stackId: string, current: PersistedProgress): PersistedProgress {
  ...
  try {
    ...
    if (phase === 'awaiting-start' || phase === 'starting' || phase === 'awaiting-reenable') {
...
export function createSetupProgressStore(stackId: string): SetupProgressStore {
  ...
  return {
    ...
    getIpPostSyncGuide: () => read().ipPostSyncGuide,
    setIpPostSyncGuide: (guide) => {
      persist(stackId, { ...read(), ipPostSyncGuide: guide })
    },
