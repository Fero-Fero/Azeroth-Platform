...
function SoapAction(ctx: SetupStepContext) {
  ...
  if (revealedPassword) {
    return setupActionButton('Continue', () => {
      ctx.status.progress.setSessionFlag(SOAP_CREDENTIALS_FLAG, null)
    }, {
...
function credentialsVisible(ctx: SetupStepContext) {
  return ctx.status.progress.getSessionFlag(SOAP_CREDENTIALS_FLAG) === '1'
}

export function soapAdminStep(): SetupStep {
  return {
    id: STEP_IDS.soapAdmin,
    level: (ctx) => (ctx.status.soapInitialized ? 'success' : 'error'),
    title: 'SOAP admin account',
    defaultExpanded: true,
    applies: (ctx) => !ctx.status.soapInitialized || credentialsVisible(ctx),
    isComplete: (ctx) => ctx.status.soapInitialized,
    showWhenComplete: (ctx) => credentialsVisible(ctx),
    summary: (ctx) =>
      ctx.status.progress.getSessionFlag(SOAP_PASSWORD_FLAG)
        ? 'Save the credentials below, then continue. A copy is in soap-credentials.txt.'
