...
import type { ModuleConfigSchema } from '@/types/module-config.types'
...
export const stackApi = {
  ...
  continueExpressProvision: (stackId: string) =>
    apiClient.post(`/stacks/${stackId}/express-provision/continue`),

  startExpressProvision: (stackId: string) =>
    apiClient.post(`/stacks/${stackId}/express-provision/start`),

  retryExpressProvision: (stackId: string) =>
    apiClient.post(`/stacks/${stackId}/express-provision/retry`),

  dismissExpressReadyNotice: (stackId: string) =>
    apiClient.post(`/stacks/${stackId}/express-provision/dismiss-ready`),
...
export const buildApi = {
  ...
  start: (
    ...
    apiClient.post<{ buildId: string; status: string }>(
      ...
      {
        params: {
          ...(configMigrationMode ? { configMigrationMode } : {}),
          ...(skipModuleCheck ? { skipModuleCheck: true } : {}),
        },
      },
    ),
  ...
  // Clone missing git modules or reset existing checkouts to the latest catalog/stack branch (no rebuild).
  syncModules: (stackId: string, moduleId?: string) =>
    apiClient.post<import('@/types/stack.types').SyncStackModulesResultDto>(
      `/stacks/${stackId}/modules/sync`,
      moduleId ? { moduleId } : {},
      { timeout: 600_000 },
    ),

  // Clone/refresh core+modules and compile each selected module CMake target (no Docker image build).
  checkModules: (stackId: string) =>
    apiClient.post<{ buildId: string; status: string }>(`/stacks/${stackId}/modules/check`),
}
...
export const moduleApi = {
  ...
  serviceEnvTemplates: () =>
    apiClient.get<import('@/types/service-env.types').ServiceEnvTemplate[]>('/service-env-templates'),
...
export const clientApi = {
  ...
  uploadBase: (stackId: string, file: File, onProgress?: (percent: number) => void) => {
    const form = new FormData()
    form.append('file', file)
    return apiClient.post<import('@/types/stack.types').ClientJobStatus>(`/stacks/${stackId}/client/base`, form, {
      headers: { 'Content-Type': 'multipart/form-data' },
      onUploadProgress: (e) => {
        if (onProgress && e.total) {
          onProgress(Math.round((e.loaded / e.total) * 100))
  ...
  downloadBase: (stackId: string, url: string) =>
    apiClient.post<import('@/types/stack.types').ClientJobStatus>(`/stacks/${stackId}/client/download`, { url }),

  rescanBase: (stackId: string) =>
    apiClient.post<import('@/types/client.types').ClientBaseInfoDto>(`/stacks/${stackId}/client/rescan`),

  // Deletes every client file the stack serves. The launcher build, portal, branding and news survive.
  purge: (stackId: string) =>
    apiClient.post<import('@/types/stack.types').ClientJobStatus>(`/stacks/${stackId}/client/purge`),
