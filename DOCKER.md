...
Stacks that select an AI chat module also start sidecar containers on the same prefix:
`…-ollama`, `…-ollama-pull` (one-shot model download), and — for LLM Chatter —
`…-llm-chatter-bridge`.
...
Stacks that select an AI chat module also attach `azeroth-platform-ollama-models` (shared across
stacks on the same Docker engine) for the LLM weights. The first start pulls `llama3.2:1b`, which
is a multi-gigabyte download.
...
Selecting Bot Buddy, Chat, or LLM Chatter adds an **Ollama** sidecar (`ollama/ollama:latest`, or
the `:rocm` tag when the stack's Docker engine exposes AMD). A one-shot `ollama-pull` job writes
the default model into `azeroth-platform-ollama-models`; the serve container then keeps it
resident. Worldserver reaches it on the internal network at `http://ollama:11434` — the port is
not published on the host.

LLM Chatter also needs a Python bridge. The manager builds `localhost/acore/ac-llm-chatter-bridge:<stackId>`
from an embedded Dockerfile during the stack build and starts `…-llm-chatter-bridge` next to
Ollama. That image is stack-scoped and is removed when the stack is deleted.
