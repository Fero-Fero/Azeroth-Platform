...
public sealed class MpqToolCli : IMpqToolCli
{
    ...
    public async Task ExtractAllAsync(string mpqPath, string outputDir, CancellationToken cancellationToken)
    {
        ...
        if (relOut.StartsWith("..", StringComparison.Ordinal))
        {
            ...
            var copied = !string.Equals(
                Path.GetFullPath(mpqPath), Path.GetFullPath(staged), StringComparison.OrdinalIgnoreCase);
            if (copied)
            {
            ...
            {
                // A client MPQ runs to gigabytes, and the copy lands beside the caller's output rather
                // than in scratch, so leaving it behind grows the data volume.
                if (copied)
                {
    ...
    private static void TryDeleteFile(string path) => TempWorkspace.TryDelete(path, isDirectory: false);
