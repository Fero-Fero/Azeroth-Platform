...
public sealed class ModuleInstallOrchestrator : IModuleInstallOrchestrator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        InstalledModulesLayout.SaveChoices(stackRoot, request);
        await _migrations.TryEnsureServerDbcBaselineAsync(stackId, cancellationToken);
        var baselineDbcDir = MigrationLayout.ServerDbcDir(stackRoot);
        using var session = new ModuleInstallSession(InstalledModulesLayout.Root(stackRoot));
        ...
        try
        {
            ...
            foreach (var moduleId in moduleIds)
            {
                ...
                var helpers = new ModuleInstallHelpers(
                    moduleId, packageRoot, session, _wdbx, _dbcStore, _mpqTool, baselineDbcDir);
    ...
    {
        var stackModule = ModuleCompileEnvironment.ModuleDirectory(
            Path.Combine(stackRoot, "azerothcore-wotlk", "modules"),
            moduleId,
            _hooks.Find(moduleId)?.Compile);
