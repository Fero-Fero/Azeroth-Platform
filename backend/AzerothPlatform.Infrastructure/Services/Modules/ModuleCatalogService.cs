...
public sealed partial class ModuleCatalogService : IModuleCatalogService
{
    ...
    [
        ...
        new()
        {
            ...
            Name = "Dungeon Clear",
            Description = "Autonomous tank-led 5-man dungeon clears for playerbots - the tank drives the party boss to boss, clears trash, and handles scripted events. Requires mod-playerbots. Uses TopHatMan's fork (auto-playerbots branch) so Playerbot Dungeon Sim can call StartAutonomousClear. Pairs with the DungeonClear client addon for in-game control.",
            Repository = "https://github.com/TopHatMan/mod-dungeon-clear.git",
            Branch = "auto-playerbots",
            IsBuiltIn = true
        },
        new()
        {
            ...
            Description = "Progression engine for random playerbots: level-appropriate 5-man dungeon runs via Dungeon Clear, then offscreen raid progression at cap with gearing from real loot tables. Requires Dungeon Clear (auto-playerbots branch, which provides StartAutonomousClear) and mod-playerbots. Set DungeonClear.AllowAutonomousBotRuns = 1; apply the module SQL to the characters database after install.",
        ...
        new()
        {
            ...
            Description = "Experimental LLM-driven playerbot control via the Ollama API (questing, grinding, chat overrides). Requires mod-playerbots. The stack runs its own Ollama container, so no external LLM server is needed. Only one AI bot chatting module can be selected. Optional BuddyBotUI debug addon appears in Addons when this module is selected.",
        ...
        new()
        {
            Id = "mod-ollama-chat",
            Name = "Ollama Chat",
            Description = "In-character playerbot chat via the Ollama API (personalities, random chatter, event remarks). Requires mod-playerbots. The stack runs its own Ollama container, so no external LLM server is needed. Only one AI bot chatting module can be selected.",
            Repository = "https://github.com/Fero-Fero/mod-ollama-chat",
            Branch = "main",
            IsBuiltIn = true,
            RequiredModuleIds = ["mod-playerbots"]
        },
        new()
        {
            Id = LlmChatterBridge.ModuleId,
            Name = "LLM Chatter",
            Description = "Roleplay conversation engine for playerbots: persistent personalities, memories and backstories, zone-aware ambient chatter, party/raid/battleground/guild dialogue. Requires mod-playerbots. The stack runs its own Ollama container plus the module's Python bridge, so no external LLM server is needed. Unlike the Ollama modules it layers on top of playerbot chatter instead of replacing it, so the built-in bot talk stays on. Only one AI bot chatting module can be selected.",
            Repository = "https://github.com/Hokken/mod-llm-chatter",
            Branch = "master",
            IsBuiltIn = true,
            RequiredModuleIds = ["mod-playerbots"]
        },
    ...
    // Git ref/branch names accepted for clone/fetch, restricted to letters, digits and path-safe
    // punctuation, never starting with '-'. A leading '-' would let a value masquerade as a git option
    // such as --upload-pack, which is argument injection into the git CLI.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Bundled modules are compiled into the fork and required modules are forced on, so both satisfy
        // a requirement despite never appearing in the catalog.
        return DropUnsatisfiableModules(
            visible,
            (_serverTypeCatalog.GetBundledModuleIds(serverType.Value) ?? [])
                .Concat(_serverTypeCatalog.GetRequiredModuleIds(serverType.Value) ?? []));
    }

    /// <summary>
    /// Hides modules whose <see cref="ModuleDto.RequiredModuleIds"/> cannot be met on this server type,
    /// so a type that drops Playerbots also drops everything built on top of it. Runs to a fixed point:
    /// a requirement may itself be hidden for the same reason.
    /// </summary>
    /// <param name="forcedIds">Ids that satisfy a requirement without being in the catalog.</param>
    internal static IReadOnlyList<ModuleDto> DropUnsatisfiableModules(
        List<ModuleDto> visible,
        IEnumerable<string> forcedIds)
    {
        var forced = new HashSet<string>(forcedIds, StringComparer.OrdinalIgnoreCase);

        var kept = visible;
        bool dropped;
        do
        {
            var satisfiable = new HashSet<string>(forced, StringComparer.OrdinalIgnoreCase);
            foreach (var module in kept)
            {
                satisfiable.Add(module.Id);
            }

            var next = kept
                .Where(module => module.RequiredModuleIds.All(satisfiable.Contains))
                .ToList();

            dropped = next.Count != kept.Count;
            kept = next;
        }
        while (dropped);
    ...
    private ModuleDto CloneWithRepository(ModuleDto source, string repository, string branch) => new()
    {
        Id = source.Id,
    ...
    private ModuleDto WithCompile(ModuleDto module)
    {
        var compile = _installHooks.Find(module.Id)?.Compile;
        if (compile is null || ReferenceEquals(compile, ModuleCompileProfile.Empty))
        {
            return module;
        }

        var copy = CloneWithRepository(module, module.Repository, module.Branch);
    ...
    public async Task<IReadOnlyList<ModuleDto>> ListAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        return BuiltInModules.Select(WithCompile).Concat(custom).ToList();
    }
