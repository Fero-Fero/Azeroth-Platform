...
internal static partial class PatchNewsReader
{
    ...
    private static readonly HashSet<string> StrippedImageExtensions = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    /// <summary>
    /// Builds a cover/inline-image id that <c>LauncherPortalService</c> will accept: ASCII letters,
    /// digits, hyphen and underscore only. File extensions are stripped so the portal can append the
    /// real one; spaces and other punctuation become hyphens.
    /// </summary>
    public static string ToPublishedAssetId(string articleId, string relativeAssetPath)
    {
        ...
        return SanitizePublishedId($"{articleId}--{token}");
    }

    /// <summary>
    /// Slug used as a launcher news item/cover id. Dots in article ids (e.g. <c>1.0</c>) become
    /// hyphens; a trailing image extension is dropped so it is not double-appended on store.
    /// </summary>
    public static string SanitizePublishedId(string value)
    {
        ...
        if (StrippedImageExtensions.Contains(extension))
        {
