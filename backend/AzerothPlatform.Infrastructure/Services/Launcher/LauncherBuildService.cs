...
public sealed class LauncherBuildService : ILauncherBuildService
{
    ...
    private async Task RunBuildAsync(LauncherVersionPart part, CancellationToken cancellationToken)
    {
        try
        {
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                // Manifests arrive over each stack's plain-HTTP channel, so the signing pubkey is always
                // baked in: it is the only thing the launcher can anchor manifest trust to.
        ...
        string signingPublicKey)
    {
        ...
        // are applied at runtime from each stack's portal. The manifest signing pubkey is baked in because
        // it is what manifest trust is anchored to, with no TLS channel back to the manager.
