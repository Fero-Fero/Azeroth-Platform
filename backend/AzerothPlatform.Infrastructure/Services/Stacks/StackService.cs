...
public sealed class StackService : IStackService
{
    ...
    private readonly IModuleInstallHookRunner _installHooks;
    ...
    public StackService(
        ...
        IModuleInstallHookRunner installHooks,
    ...
    public async Task<StackDetailsDto> CreateAsync(StackConfigurationDto configuration, CancellationToken cancellationToken = default)
    {
        ...
        stack.ModuleBranchesJson = JsonSerializer.Serialize(SanitizeModuleBranches(configuration.ModuleBranches), JsonOptions);
        ...
        if (configuration.ServerType == ServerType.Express)
        {
            ...
            stack.ExpressProvisionPhase = ExpressProvisionPhase.None;
            stack.ExpressProvisionMessage = "After the first build, click Setup and Launch! on Overview.";
    ...
    public async Task<StackDetailsDto?> UpdateAsync(string stackId, StackConfigurationDto configuration, CancellationToken cancellationToken = default)
    {
        ...
        var oldBranches = ModuleBranchResolver.Parse(stack.ModuleBranchesJson);
        var newBranches = SanitizeModuleBranches(configuration.ModuleBranches);
        var branchesChanged = !ModuleBranchesEqual(oldBranches, newBranches);
        ...
        stack.ModuleIdsJson = JsonSerializer.Serialize(configuration.ModuleIds, JsonOptions);
        stack.ModuleBranchesJson = JsonSerializer.Serialize(newBranches, JsonOptions);
        if (modulesChanged || branchesChanged)
        {
            InvalidateModuleCheck(stack);
        }
        stack.AddonIdsJson = JsonSerializer.Serialize(configuration.AddonIds ?? [], JsonOptions);
        // MapAsync withholds the root password from the details payload, so a blank value on update
        ...
        if (modulesChanged)
        {
            ...
            BuildService.RemoveUnselectedModuleDirectories(
                modulesPath,
                ModuleCompileEnvironment.ModuleDirectoriesToKeep(newModuleIds, _installHooks.All));
        }

        if (branchesChanged)
        {
            await CheckoutChangedModuleBranchesAsync(
                stackId, newModuleIds, oldBranches, newBranches, cancellationToken);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (servicesToRecreate.Contains("ac-worldserver", StringComparer.OrdinalIgnoreCase))
        {
            await StartOllamaSidecarInBackgroundAsync(stack, repoPath, cancellationToken);
        }
    ...
    public async Task<bool> StartAsync(string stackId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Stack start does not bring up frontend-armory (that is a dedicated Start Armory
            // action). Compose still includes the armory so armory-assets can start when the
            // dataset exists; waiting for the armory image here would block game servers.
            var includeArmoryCompose = stack.IncludeArmory;
            ...
            await EnsureRuntimeConfigurationAsync(stack, repoPath, cancellationToken, includeArmory: includeArmoryCompose, includeClient: clientReady);
            ...
            await ShipExternalStackImagesAsync(stack, includeArmory: false, clientReady, cancellationToken);
            await BringStackUpAsync(
                stack,
                stackId,
                repoPath,
                armoryReady: false,
    ...
    public async Task<bool> ForceStopAsync(string stackId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await RunDockerComposeAsync(
                stackId,
                "stop -t 5 ac-authserver ac-worldserver frontend-armory armory-assets client ollama ollama-pull llm-chatter-bridge",
                repoPath,
                cancellationToken,
                throwOnError: false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await RunDockerComposeAsync(
            stackId,
            "stop ac-authserver ac-worldserver frontend-armory armory-assets client ollama ollama-pull llm-chatter-bridge",
            repoPath,
            cancellationToken,
            throwOnError: false);
        ...
        if (initServices.Count > 0)
        {
            ...
            // --force-recreate: a previous failed db-import stays around after rm fails, and
            // compose up would reuse it without the modules volume.
            await RunDockerComposeAsync(
                stackId,
                $"up -d --force-recreate {string.Join(' ', initServices)}",
                repoPath,
                cancellationToken);
            if (!dbImportDone)
            {
        ...
        await RunDockerComposeAsync(
            ...
            repoPath,
            cancellationToken);
        // Do not await the image/model pull — it can take a long time and Express SOAP only needs
        // auth/world. Capture Docker context while this scope's DbContext is alive, then run compose
        // detached so a disposed request scope cannot throw ObjectDisposedException.
        await EnqueueOllamaSidecarStartAsync(stack, repoPath, cancellationToken);
    }
    ...
    {
        var contextArg = await GetDockerContextArgAsync(stackId, cancellationToken);
        var (inspectExit, inspectOut, _) = await RunDockerCliAsync(
            $"{contextArg}inspect -f \"{{{{.State.Status}}}}|{{{{.State.ExitCode}}}}\" {dbImportContainerName}",
            cancellationToken);
        if (inspectExit == 0)
        {
            var parts = inspectOut.Trim().Split('|');
            var status = parts.Length > 0 ? parts[0].Trim() : string.Empty;
            var exitCode = parts.Length > 1 && int.TryParse(parts[1].Trim(), out var parsed) ? parsed : -1;
            if (status.Equals("exited", StringComparison.OrdinalIgnoreCase))
            {
                // A failed import still leaves world tables; only a zero exit means we can skip a rerun.
                return exitCode == 0;
            }
    ...
    private void ReportLifecycleProgress(string stackId, string message, bool persistExpress = true)
    {
        ...
        var stack = _dbContext.ManagedStacks.Local.FirstOrDefault(item => item.Id == stackId);
        if (stack is not
            {
                ServerType: ServerType.Express,
                ExpressProvisionStatus: ExpressProvisionStatus.Running,
                ExpressProvisionPhase: ExpressProvisionPhase.StartStack
            })
        {
            return;
        }

        stack.ExpressProvisionMessage = message.Length <= 512 ? message : message[..512];
        try
        {
        ...
        {
            _logger.LogDebug(ex, "Could not persist Express start progress for stack {StackId}.", stackId);
        }
    }
            ...
            if (process is not null)
            {
                ...
                if (process.ExitCode == 0)
                {
                    ...
                    if (status.Equals("exited", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (code != 0)
                        {
                            var logs = await TryReadContainerLogsAsync(stackId, containerName, tail: 200, cancellationToken);
        ...
        bool throwOnError = true)
    {
        ...
        await RunDockerComposeResolvedAsync(
            stackId, dockerContext, arguments, workingDirectory, cancellationToken, throwOnError);
    }

    private async Task RunDockerComposeResolvedAsync(
        string stackId,
        ...
        bool preferCachedRuntimeProbe = false)
    {
        ...
        var skipRuntimeProbe = StackRuntimeProbePolicy.ShouldSkipLiveProbe(
            activeJob is { IsRunning: true },
            stack.DeploymentTarget,
            stack.Status);
        ...
        // Overlay Starting while a start-like job is in flight and the probe has not yet reached
        // Running/Initializing (external stacks skip docker ps during the job, so this is the
        // usual case there). Local first-time setup keeps Initializing once db-import is up.
        ...
        // SOAP admin reconcile hits MySQL; skip it while a lifecycle job (including first-time
        // db-import) is already using the database.
        if (probeRuntime && !skipRuntimeProbe && activeJob is not { IsRunning: true } && canAttemptSoapReconcile)
        {
        ...
        return new StackDetailsDto
        {
            ...
            ExpressProvisionPhase = stack.ExpressProvisionPhase,
            ExpressGameAccountCreated = stack.ExpressGameAccountCreated
                || (stack.ServerType == ServerType.Express
                    && stack.ExpressProvisionStatus is ExpressProvisionStatus.WaitingForClient
                        or ExpressProvisionStatus.Completed),
            ExpressReadyNoticePending = stack.ExpressReadyNoticePending,
            ModuleCheck = MapModuleCheck(stack),
        };
    }

    private static ModuleCheckStatusDto? MapModuleCheck(ManagedStackEntity stack)
    {
        ...
        try
        {
            ...
            dto.Passed = dto.Passed && !string.IsNullOrEmpty(stack.ModuleCheckFingerprint);
            dto.Fingerprint = string.IsNullOrEmpty(stack.ModuleCheckFingerprint) ? null : stack.ModuleCheckFingerprint;
            return dto;
        }
        catch (JsonException)
        {
    ...
    private static Dictionary<string, string> SanitizeModuleBranches(Dictionary<string, string>? branches)
    {
        ...
        foreach (var (key, value) in branches)
        {
            ...
            result[key.Trim()] = ModuleCatalogService.ValidateGitRef(value.Trim());
        }

        return result;
    }

    private static bool ModuleBranchesEqual(
    ...
    private static void InvalidateModuleCheck(ManagedStackEntity stack)
    {
        ...
        {
            stack.ModuleCheckJson = string.Empty;
        }
    }

    private async Task CheckoutChangedModuleBranchesAsync(
        string stackId,
        IReadOnlyCollection<string> selectedIds,
        IReadOnlyDictionary<string, string> oldBranches,
        IReadOnlyDictionary<string, string> newBranches,
        CancellationToken cancellationToken)
    {
        var modulesPath = Path.Combine(GetStackPath(stackId), "azerothcore-wotlk", "modules");
        ...
        {
            if (!newBranches.TryGetValue(moduleId, out var nextBranch) || string.IsNullOrWhiteSpace(nextBranch))
            {
                continue;
            }

            oldBranches.TryGetValue(moduleId, out var previousBranch);
            if (string.Equals(previousBranch, nextBranch, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            var moduleDir = ModuleCompileEnvironment.ModuleDirectory(
                modulesPath,
                moduleId,
                _installHooks.Find(moduleId)?.Compile);
            var gitDir = Path.Combine(moduleDir, ".git");
            if (!Directory.Exists(moduleDir) || (!Directory.Exists(gitDir) && !File.Exists(gitDir)))
            {
                continue;
            }

            var safeBranch = ModuleCatalogService.ValidateGitRef(nextBranch);
            await RunGitAsync(["fetch", "--depth", "1", "origin", safeBranch], moduleDir, cancellationToken);
            await RunGitAsync(["reset", "--hard", "FETCH_HEAD"], moduleDir, cancellationToken);
            await RunGitAsync(["clean", "-ffd"], moduleDir, cancellationToken);
            _logger.LogInformation(
                "Checked out module {ModuleId} on branch {Branch} for stack {StackId}",
                moduleId, safeBranch, stackId);
        }
    }
        ...
        if (process.ExitCode != 0)
        {
            throw new InvalidOperationException(
                string.IsNullOrWhiteSpace(stderr)
                    ? $"git {arguments[0]} exited with code {process.ExitCode}."
                    : stderr);
        }
    }
    ...
    private List<string> GetModulesPendingRebuild(string stackId, List<string> moduleIds)
    {
        ...
        return moduleIds
            .Where(id => !Directory.Exists(ModuleCompileEnvironment.ModuleDirectory(
                modulesPath,
                id,
                _installHooks.Find(id)?.Compile)))
            .ToList();
    }
    ...
    private static readonly Dictionary<string, (string DisplayName, string Category)> ExtraServiceMeta = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["ollama-pull"] = ("Ollama Pull", "init"),
        [LlmChatterBridge.ServiceName] = ("LLM Chatter Bridge", "sidecar"),
    };
    ...
    private static readonly HashSet<string> ControllableServices = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "ac-db-import", "ac-client-data-init", "ac-tools", "ac-dev-server", "ollama", "ollama-pull",
    };
    ...
    private static string GuessServiceFromName(string name)
    {
        ...
        if (name.EndsWith("-ollama-pull", StringComparison.OrdinalIgnoreCase)) { return "ollama-pull"; }
        if (name.EndsWith("-ollama", StringComparison.OrdinalIgnoreCase)) { return "ollama"; }
        if (name.EndsWith("-llm-chatter-bridge", StringComparison.OrdinalIgnoreCase)) { return LlmChatterBridge.ServiceName; }
        return string.Empty;
    }
        ...
        bool? includeClient = null)
    {
        ...
        // Generate and persist a random session secret on first use (independent of the DB password)
        // so it can't be recomputed by anyone who only learns the DB credentials. The same value is
        // the client-server's CLIENT_AUTH_TOKEN, so it must exist whenever either service is rendered
        // — not only when the armory is included. Otherwise the client starts with a throwaway token
        // the manager never stores, and later POST /rescan calls 401.
        if ((renderArmory || renderClient) && string.IsNullOrEmpty(stack.ArmorySessionSecret))
        {
        ...
        var ollama = await ResolveOllamaComposeOptionsAsync(stack, cancellationToken);
        await File.WriteAllTextAsync(environmentPath, environment.ToString(), cancellationToken);
        await File.WriteAllTextAsync(
            overridePath,
            GenerateRuntimeDockerComposeOverride(
                stack, composeProjectName, repoPath, renderArmory, renderClient, ollama),
            cancellationToken);

        // Stamping the template version is how "deployment drift" is cleared: a stack generated by an
        // older manager carries a lower version and is surfaced as "re-apply required" until fresh
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var modulesVolume = DockerComposeOverrideGenerator.ModulesVolumeName(stack.Id);
            var rewritten = _installHooks.PrepareCheckouts(modulesDir);
            if (rewritten.Count > 0)
            {
                _logger.LogInformation(
                    "Rewrote {Count} module SQL file(s) via install hooks so db-import can apply them (stack {StackId}).",
                    rewritten.Count,
                    stack.Id);
            }

            MakeTreeWorldReadable(modulesDir);
            await _remoteEngine.SeedVolumeAsync(stack, modulesVolume, modulesDir, cancellationToken);

            // Seed is a tree copy that can miss a just-rewritten file if the daemon-side copy
            // races the manager's write. Push each rewritten file into the volume explicitly.
            foreach (var file in rewritten)
            {
                var relative = Path.GetRelativePath(modulesDir, file).Replace('\\', '/');
                await using var stream = File.OpenRead(file);
                await _remoteEngine.WriteVolumeFileFromStreamAsync(
                    stack, modulesVolume, relative, stream, cancellationToken);
            }

            // db-import and worldserver run as uid 1000; seeded trees keep the manager uid.
            await _remoteEngine.SetVolumeOwnershipAsync(
                stack, modulesVolume, AcoreServiceUid, AcoreServiceGid, cancellationToken);
            await _remoteEngine.SetVolumeWorldReadableAsync(stack, modulesVolume, cancellationToken);
        }

        // Server config (etc): copy every .conf.dist to a sibling .conf (modules + worldserver +
        // authserver) before seeding the volume, so Express / Server Wide Progression can edit configs
        // without waiting for the game servers to generate them.
        var etcDir = Path.Combine(repoPath, "env", "dist", "etc");
        Directory.CreateDirectory(etcDir);
        var leftoverDbImportWrapper = Path.Combine(etcDir, "azp-dbimport.sh");
        if (File.Exists(leftoverDbImportWrapper))
        {
            File.Delete(leftoverDbImportWrapper);
        }

        await _serverConfig.EnsureSeededAsync(stack.Id, cancellationToken);
        var runtimeSidecars = RuntimeSidecarsFor(stack);
        var moduleIds = ModuleIdsFor(stack);
        ModuleSidecarConf.Apply(etcDir, runtimeSidecars);
        if (ModuleCompileEnvironment.HasLlmChatterBridge(runtimeSidecars)
            && AiChatSidecarNeed.LlmChatterBridgeContainer(etcDir, moduleIds))
        {
            LlmChatterConf.Apply(
                etcDir,
                ModuleCompileEnvironment.OllamaSidecarFor(runtimeSidecars)?.Model ?? OllamaSidecar.Model,
                DockerComposeOverrideGenerator.InternalDatabaseHost,
                DockerComposeOverrideGenerator.InternalDatabasePort,
                "root",
                stack.DatabaseRootPassword,
                "acore_characters");
            await SidecarImageShipping.ShipToStackEngineIfNeededAsync(
                stack, _remoteEngine, LlmChatterBridge.ImageTag(stack.Id), cancellationToken);
        }
        ...
        var ollamaSidecar = ModuleCompileEnvironment.OllamaSidecarFor(runtimeSidecars);
        if (ollamaSidecar is not null
            && AiChatSidecarNeed.Ollama(etcDir, moduleIds)
            && !string.IsNullOrWhiteSpace(ollamaSidecar.ModelsVolumeName))
        {
            await _remoteEngine.EnsureVolumeExistsAsync(stack, ollamaSidecar.ModelsVolumeName, cancellationToken);
        }
        ...
        var luaDir = Patches.MigrationLayout.LuaScriptsDir(stackRoot);
    ...
    private string GenerateRuntimeDockerComposeOverride(
        ManagedStackEntity stack,
        string composeProjectName,
        string repoPath,
        bool includeArmory,
        bool includeClient,
        OllamaComposeOptions? ollama)
    {
        ...
        var llmChatterBridge = AiChatSidecarNeed.LlmChatterBridgeContainer(
                Path.Combine(repoPath, "env", "dist", "etc"),
                ModuleIdsFor(stack))
            ? LlmChatterBridgeComposeOptions.ForStack(stack.Id)
            : null;
        ...
        return DockerComposeOverrideGenerator.Generate(
            ...
            ollama,
            managerDataVolumeName,
            modulesSubpath,
            llmChatterBridge);
    }

    private IReadOnlyList<string> ModuleIdsFor(ManagedStackEntity stack) =>
        Deserialize<List<string>>(stack.ModuleIdsJson) ?? [];

    private IReadOnlyList<ModuleRuntimeSidecar> RuntimeSidecarsFor(ManagedStackEntity stack) =>
        ModuleCompileEnvironment.RuntimeSidecarsFor(ModuleIdsFor(stack), _installHooks.All);

    private string StackEtcDir(string stackId) =>
        Path.Combine(GetStackPath(stackId), "azerothcore-wotlk", "env", "dist", "etc");

    private async Task<OllamaComposeOptions?> ResolveOllamaComposeOptionsAsync(
        ManagedStackEntity stack,
        CancellationToken cancellationToken)
    {
        if (!AiChatSidecarNeed.Ollama(StackEtcDir(stack.Id), ModuleIdsFor(stack)))
        {
            return null;
        }

        var sidecar = ModuleCompileEnvironment.OllamaSidecarFor(RuntimeSidecarsFor(stack));
        if (sidecar is null)
        {
            return null;
        }
        ...
        var contextArg = await GetDockerContextArgAsync(stack.Id, cancellationToken);
        var gpu = await OllamaGpuProbe.ProbeAsync(contextArg, _logger, cancellationToken);
        return OllamaComposeOptions.FromSidecar(sidecar, gpu);
    }

    private sealed record OllamaSidecarStartPlan(
        string StackId,
        string? DockerContext,
        string Image,
        string Model,
    ...
    private async Task<OllamaSidecarStartPlan?> TryCreateOllamaStartPlanAsync(
        ManagedStackEntity stack,
        CancellationToken cancellationToken)
    {
        if (!AiChatSidecarNeed.Ollama(StackEtcDir(stack.Id), ModuleIdsFor(stack)))
        ...
        var dockerContext = await ResolveDockerContextAsync(stack.Id, cancellationToken);
        var options = OllamaComposeOptions.FromSidecar(sidecar, GpuBackend.Cpu);
        return new OllamaSidecarStartPlan(
            stack.Id,
            dockerContext,
            string.IsNullOrWhiteSpace(sidecar.Image) ? OllamaSidecar.Image : sidecar.Image,
            options.Model,
            options.ModelsVolumeName);
    }

    /// <summary>
    /// Resolves Docker context while the current DbContext is alive, then starts Ollama without
    /// touching EF. Used when stack Start returns before the sidecar is up. An Enable = 0 module
    /// conf leaves the sidecar off and removes any leftover containers so they cannot keep VRAM.
    /// </summary>
    private async Task EnqueueOllamaSidecarStartAsync(
        ManagedStackEntity stack,
        string repoPath,
        CancellationToken cancellationToken)
    {
        await RemoveUnusedAiSidecarContainersAsync(stack, cancellationToken);

        OllamaSidecarStartPlan? plan;
        try
        {
            plan = await TryCreateOllamaStartPlanAsync(stack, cancellationToken);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(ex, "Ollama sidecar start failed for stack {StackId}.", stack.Id);
            return;
        }
        ...
        _ = ExecuteOllamaSidecarStartAsync(plan, repoPath, persistExpressProgress: false, CancellationToken.None);
    }

    /// <summary>
    /// Starts the Ollama sidecar after worldserver. Pull is skipped when the model is already on
    /// the shared models volume. Serve always runs (<c>up -d --no-deps ollama</c>) even if pull
    /// fails, so a hung or failed pull cannot leave the stack without a chat backend.
    /// </summary>
    private async Task StartOllamaSidecarInBackgroundAsync(
        ManagedStackEntity stack,
        string repoPath,
        CancellationToken cancellationToken)
    {
        await RemoveUnusedAiSidecarContainersAsync(stack, cancellationToken);
        var plan = await TryCreateOllamaStartPlanAsync(stack, cancellationToken);
        if (plan is null)
        {
            return;
        }

        await ExecuteOllamaSidecarStartAsync(plan, repoPath, persistExpressProgress: true, cancellationToken);
    }

    /// <summary>
    /// Drops Ollama / LLM Chatter containers that the current conf will not use.
    /// <c>restart: unless-stopped</c> would otherwise keep a leftover sidecar alive after Enable
    /// is turned off and the service is omitted from compose.
    /// </summary>
    private async Task RemoveUnusedAiSidecarContainersAsync(
        ManagedStackEntity stack,
        CancellationToken cancellationToken)
    {
        var etcDir = StackEtcDir(stack.Id);
        var moduleIds = ModuleIdsFor(stack);
        var keepOllama = AiChatSidecarNeed.Ollama(etcDir, moduleIds);
        var keepBridge = AiChatSidecarNeed.LlmChatterBridgeContainer(etcDir, moduleIds);
        if (keepOllama && keepBridge)
        {
            return;
        }
        ...
        if (!keepOllama)
        {
            await RemoveStackContainerByServiceAsync(stack, contextArg, OllamaSidecar.ServiceName, cancellationToken);
            await RemoveStackContainerByServiceAsync(
                stack, contextArg, DockerComposeOverrideGenerator.OllamaPullService, cancellationToken);
        }

        if (!keepBridge)
        {
            await RemoveStackContainerByServiceAsync(stack, contextArg, LlmChatterBridge.ServiceName, cancellationToken);
        }
    }

    private async Task RemoveStackContainerByServiceAsync(
        ManagedStackEntity stack,
        string contextArg,
        string composeService,
        CancellationToken cancellationToken)
    ...
    private async Task ExecuteOllamaSidecarStartAsync(
        OllamaSidecarStartPlan plan,
        string repoPath,
        bool persistExpressProgress,
        CancellationToken cancellationToken)
    {
        try
        {
            var contextArg = plan.DockerContext is null ? string.Empty : $"--context {plan.DockerContext} ";
            var gpu = await OllamaGpuProbe.ProbeAsync(contextArg, _logger, cancellationToken);
            var image = OllamaSidecar.ResolveImage(plan.Image, gpu);
            ReportLifecycleProgress(
                plan.StackId,
                $"Ollama is starting in the background ({image} / {plan.Model}). This download can take several minutes.",
                persistExpressProgress);

            var hasModel = await OllamaGpuProbe.ModelVolumeHasManifestAsync(
                contextArg, plan.Volume, plan.Model, _logger, cancellationToken);
            ...
            if (!hasModel)
            {
                try
                {
                    await RunDockerComposeResolvedAsync(
                        plan.StackId,
                        plan.DockerContext,
                        $"--profile {DockerComposeOverrideGenerator.OllamaPullService} run -T --rm --no-deps {DockerComposeOverrideGenerator.OllamaPullService}",
                        repoPath,
                        cancellationToken);
                }
                catch (Exception ex) when (ex is not OperationCanceledException)
                {
                    _logger.LogError(
                        ex,
                        "Ollama model pull failed for stack {StackId} ({Model}). Starting serve if the volume already has the model.",
                        plan.StackId,
                        plan.Model);
                }
            }
            ...
            {
                _logger.LogInformation(
                    "Skipping Ollama pull for stack {StackId}; {Model} is already on {Volume}.",
                    plan.StackId,
                    plan.Model,
                    plan.Volume);
            }
            ...
            try
            {
                await RunDockerComposeResolvedAsync(
                    plan.StackId,
                    plan.DockerContext,
                    $"up -d --no-deps {DockerComposeOverrideGenerator.OllamaService}",
                    repoPath,
                    cancellationToken);
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                _logger.LogError(ex, "Ollama serve failed to start for stack {StackId}.", plan.StackId);
            }
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(ex, "Ollama sidecar start failed for stack {StackId}.", plan.StackId);
        }
    }
    ...
    /// <summary>
    /// db-import runs as uid 1000. The manager writes module SQL as its own uid; make the tree
    /// readable so a data-volume subpath mount works without a chown of the live checkout.
    /// </summary>
    private static void MakeTreeWorldReadable(string root)
    {
        if (!Directory.Exists(root) || OperatingSystem.IsWindows())
        {
            return;
        }
        ...
        try
        {
            File.SetUnixFileMode(root, dirMode);
        ...
        {
            // Best-effort: named-volume chown still covers the seeded copy.
        }
    }
    ...
    private ClientComposeOptions BuildClientComposeOptions(ManagedStackEntity stack)
    {
        ...
        return new ClientComposeOptions
        {
            ...
            AuthToken = stack.ArmorySessionSecret,
    ...
    private ArmoryComposeOptions BuildArmoryComposeOptions(ManagedStackEntity stack)
    {
        ...
        return new ArmoryComposeOptions
        {
            ...
            ClientPortalUrl = stack.ClientEnabled
                ...
                : string.Empty,
            // Populated by EnsureRuntimeConfigurationAsync before the override is rendered.
            SessionSecret = stack.ArmorySessionSecret,
    ...
    /// <summary>
    /// Generates a fresh, cryptographically-random secret used both to sign the armory's player session
    /// cookies and as the client-server's <c>CLIENT_AUTH_TOKEN</c>. Persisted per stack (see
    /// <see cref="ManagedStackEntity.ArmorySessionSecret"/>) so it is stable across restarts yet
    /// unrelated to any other credential.
    /// </summary>
