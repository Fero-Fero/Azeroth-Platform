...
public sealed class StackConfigurationValidator : IStackConfigurationValidator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var selectedModules = configuration.ModuleIds
            .Select(id => modulesById.TryGetValue(id, out var selected) ? selected : null)
            .Where(module => module is not null)
            .Cast<ModuleDto>()
            .ToList();
        foreach (var (leftId, rightId) in ModuleCompileEnvironment.ConflictingPairs(selectedModules))
        {
            var leftName = modulesById.TryGetValue(leftId, out var left) ? left.Name : leftId;
            var rightName = modulesById.TryGetValue(rightId, out var right) ? right.Name : rightId;
            AddError(result, "moduleIds", $"{leftName} and {rightName} cannot be selected together.");
        }
