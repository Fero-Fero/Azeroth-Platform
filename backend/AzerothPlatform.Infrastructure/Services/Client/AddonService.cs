...
public sealed class AddonService : IAddonService
{
    ...
    [
        ...
        new()
        {
            ...
            RelatedModuleIds = ["mod-ollama-bot-buddy"],
        },
        new()
        {
    ...
    public async Task<AddonListDto> InstallFromCatalogAsync(string? stackId, string addonId, CancellationToken cancellationToken = default)
    {
        ...
        using var scratch = TempWorkspace.CreateFile("addon-download", ".zip");
        ...
        using (var response = await http.GetAsync(uri, HttpCompletionOption.ResponseHeadersRead, cancellationToken))
        {
            ...
            await using var dst = File.Create(scratch.Path);
            await src.CopyToAsync(dst, cancellationToken);
        }

        var folderCount = InstallArchive(scratch.Path, addonsDir, entry.InstallAsFolder, cancellationToken);
        _logger.LogInformation(
    ...
    private static async Task<int> ExtractZipAsync(Stream zipContent, string destDir, CancellationToken cancellationToken)
    {
        ...
        using var scratch = TempWorkspace.CreateFile("addon-upload", ".zip");
        await using (var fs = File.Create(scratch.Path))
        {
            await zipContent.CopyToAsync(fs, cancellationToken);
        }

        var extractedFiles = ExtractArchiveToDir(scratch.Path, destDir, "addons directory", cancellationToken);
        if (extractedFiles == 0)
        {
    ...
    {
        using var scratch = TempWorkspace.CreateDirectory("addon-stage");
        var staging = scratch.Path;
