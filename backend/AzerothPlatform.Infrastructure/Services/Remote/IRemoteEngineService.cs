...
public interface IRemoteEngineService
{
    ...
    /// <summary>
    /// Removes every volume whose name starts with <paramref name="prefix"/> that is not currently in
    /// use, and returns how many went. Sweeps throwaway staging volumes orphaned by a crashed or killed
    /// job; volumes still mounted by a running container are left alone because Docker refuses them.
    /// </summary>
    Task<int> RemoveUnusedVolumesByPrefixAsync(
    ...
    /// <summary>
    /// Extracts an archive that already sits inside <paramref name="workVolume"/> straight into
    /// <paramref name="destinationVolume"/> using a throwaway container on the stack's engine, and makes
    /// the destination match the archive: files the archive did not contain are removed rather than
    /// left behind. Platform-managed content (letter patch MPQs, addons) is excluded, because it is
    /// served from the per-stack overlay. Runs entirely engine-side, so a multi-GB client never streams
    /// back through the manager.
    /// </summary>
    Task<VolumeArchiveInstallResult> ExtractArchiveVolumeIntoVolumeAsync(
        ManagedStackEntity stack,
        string workVolume,
    ...
    /// volume-to-volume copy when the source lives inside the manager's data volume, then a bind-mount
    /// copy when the manager is running on the host, otherwise a local tar stream. Existing content in
    /// the volume is left in place (files are overwritten, not purged).
    /// </summary>
...
/// <summary>
/// Outcome of installing an archive into a volume.
/// </summary>
/// <param name="FilesInstalled">Files the archive contributed to the destination.</param>
/// <param name="FilesPurged">Pre-existing files removed because the archive did not contain them.</param>
public readonly record struct VolumeArchiveInstallResult(int FilesInstalled, int FilesPurged);
