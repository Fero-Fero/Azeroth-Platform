...
public sealed class ArmoryImageService : IArmoryImageService
{
    ...
    private async Task BuildImageAsync(string stackId, CancellationToken cancellationToken)
    {
        ...
        using (var scope = _scopeFactory.CreateScope())
        {
            ...
            // Flat under the sweepable root: a per-stack parent directory would outlive the staging
            // directory it holds and accumulate one empty folder per stack ever built.
            var staticStaging = Path.Combine(
                TempWorkspace.Root, $"armory-image-static-{stackId}-{Guid.NewGuid():N}");
            try
            {
