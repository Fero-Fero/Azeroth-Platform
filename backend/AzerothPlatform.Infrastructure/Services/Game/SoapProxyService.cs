...
public class SoapProxyService : ISoapProxyService
{
    ...
    public async Task<bool> IsReachableAsync(string stackId, CancellationToken cancellationToken = default)
    {
        var stack = await _dbContext.ManagedStacks
            .AsNoTracking()
            .SingleOrDefaultAsync(s => s.Id == stackId, cancellationToken);
        ...
        if (stack.DeploymentTarget == DeploymentTarget.External)
        {
            dockerContext = await _remoteEngine.EnsureContextAsync(stack, cancellationToken);
        }

        var running = await IsContainerRunningAsync(dockerContext, containerName, cancellationToken);
        if (running != true)
        {
            _logger.LogDebug("Worldserver is not running for stack {StackId}; SOAP is unavailable.", stackId);
            return false;
        }
        ...
        try
        {
            if (stack.DeploymentTarget == DeploymentTarget.External)
            {
                return await TryPingViaDockerExecAsync(dockerContext!, containerName, cancellationToken);
            }

            return await TryHttpPingAsync($"http://{_soapHost}:{stack.SoapPort}/", cancellationToken);
        }
        ...
        {
            _logger.LogDebug(ex, "SOAP probe failed for stack {StackId}", stackId);
            return false;
        }
    }
    ...
    private async Task<bool> TryHttpPingAsync(string soapUrl, CancellationToken cancellationToken)
    {
        var client = _httpClientFactory.CreateClient();
        client.Timeout = TimeSpan.FromSeconds(3);
        using var request = new HttpRequestMessage(HttpMethod.Post, soapUrl)
        {
            Content = new StringContent(BuildSoapEnvelope("server info"), Encoding.UTF8, "text/xml")
        };
    ...
    private static async Task<bool> TryPingViaDockerExecAsync(
        string dockerContext,
        string containerName,
        CancellationToken cancellationToken)
    {
        ...
        return stdout.Trim() is { Length: > 0 } code
            && code != "000"
