...
public sealed partial class MigrationService : IMigrationService
{
    ...
    public async Task<MigrationOverviewDto> GetOverviewAsync(string stackId, CancellationToken cancellationToken = default)
    {
        ...
        var validationRequired = hasIpModule
            && stack.ServerType != ServerType.Express
            && (ipSettings?.Bootstrapped ?? false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var stackRoot = GetStackRoot(stackId);
        using var archiveScratch = TempWorkspace.CreateFile("patch-import", ".archive");
        using var extractScratch = TempWorkspace.CreateDirectory("patch-import-extract");
        using var stackRootScratch = TempWorkspace.CreateDirectory("patch-import-root");
        var tempArchive = archiveScratch.Path;
        var tempExtract = extractScratch.Path;
        var tempStackRoot = stackRootScratch.Path;
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var (sourcePath, segments) in importableFiles)
        {
            ...
            if (normalizedCategory.Equals("mpq", StringComparison.OrdinalIgnoreCase)
                && Path.GetExtension(destination).Equals(".mpq", StringComparison.OrdinalIgnoreCase)
    ...
    public async Task<int> DeleteAllPatchesAsync(string stackId, CancellationToken cancellationToken = default)
    {
        ...
        // Dropping folders without clearing the applied index leaves reimported patches with the
        // same numbers looking "Applied", which greys out delete/drop in the UI.
        stack.AppliedPatchLevel = 0;
        stack.AppliedPatchesJson = "[]";
        ...
        string stackId, string patchKey, string category, string fileName, Stream content, string? description = null, CancellationToken cancellationToken = default)
    {
        ...
        var isMpqArchive = isMpq
            && Path.GetExtension(destination).Equals(".mpq", StringComparison.OrdinalIgnoreCase);

        // MPQ archives can be large opaque blobs, so require the uploader to describe their contents.
        // Loose pack files (Data/..., Interface/...) do not need a description.
        if (isMpqArchive && string.IsNullOrWhiteSpace(description))
        {
    ...
    public async Task DeleteFileAsync(string stackId, string patchKey, string category, string fileName, CancellationToken cancellationToken = default)
    {
        ...
        // Remove emptied container folders so they stop showing in the UI.
        if (AllowsContainers(normalizedCategory))
        {
            var baseDir = Path.GetFullPath(ResolveCategoryDir(stackRoot, patchKey, category));
            var parent = Path.GetDirectoryName(path);
            while (parent is not null
                && Directory.Exists(parent)
                && !string.Equals(Path.GetFullPath(parent), baseDir, StringComparison.Ordinal)
                && Path.GetFullPath(parent).StartsWith(baseDir, StringComparison.Ordinal)
    ...
    private static MpqConstructionPlanDto ResolveMpqConstructionPlan(string stackRoot, IReadOnlyList<PatchInfo> appliedPatches)
    {
        ...
        foreach (var patch in appliedPatches.OrderBy(p => p.Level))
        {
            ...
            foreach (var name in MpqPackFilter.ConstructedArchiveNames(manifest))
            {
        ...
        foreach (var (name, (patchKey, desc)) in lastAddByName)
        {
            ...
            var mpqPath = Path.Combine(mpqDir, name);
            plan.ToBuild.Add(new MpqConstructionEntryDto
            {
                MpqName = name,
                PatchKey = patchKey,
                Description = desc,
                PreBuilt = File.Exists(mpqPath) && !MpqPackFilter.HasPackableContentFor(mpqDir, name)
            });
        }
    ...
    public async Task<List<PublishedMpqDto>> GetPublishedMpqsAsync(string stackId, CancellationToken cancellationToken = default)
    {
        ...
        // Only MPQs belonging to a successfully-applied patch (level <= current applied level) count as
        // published. Archives a failed apply copied into the overlay before erroring out mid-run are
        // excluded: they are not published until that patch actually applies.
    ...
    private static bool AllowsDeepNesting(string normalizedCategory) =>
        string.Equals(normalizedCategory, "mpq", StringComparison.OrdinalIgnoreCase);

    private static bool AllowsContainers(string normalizedCategory) =>
        AllowsDeepNesting(normalizedCategory)
    ...
    private static bool CategoryAccepts(string normalizedCategory, string fileName)
    {
        ...
        return normalizedCategory switch
        {
            ...
            "mpq" => AcceptsMpqCategoryFile(fileName),
    ...
    internal static bool AcceptsMpqCategoryFile(string fileName)
    {
        var name = Path.GetFileName(fileName);
        if (name.Equals(MpqPackFilter.MpqManifestFileName, StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }
        ...
        return MpqPackFilter.ShouldIncludeInConstructedMpq(name);
    }

    /// <summary>
    /// Enumerates a category's files: at the directory root plus (for container categories) inside
    /// exactly one level of sub-folders. MPQ content may be nested arbitrarily (WoW Data/Interface trees).
    /// </summary>
    private static IEnumerable<string> EnumerateCategoryFiles(string dir, string normalizedCategory)
    {
        ...
        if (AllowsDeepNesting(normalizedCategory))
        {
    ...
    private static (int Sql, int Dbc, int Map, int Mpq) CountFiles(string stackRoot, string patchKey)
    {
        ...
        var mpq = EnumerateCategoryFiles(MigrationLayout.MpqDir(stackRoot, patchKey), "mpq")
            .Count(p => MpqPackFilter.IsValidConstructedMpqName(Path.GetFileName(p)));
    ...
    private static (string? Subfolder, string FileName) ParseCategoryPath(string normalizedCategory, string relativePath)
    {
        ...
        if (segments.Length >= 2)
        {
            if (AllowsDeepNesting(normalizedCategory))
            {
                ...
                subfolder = string.Join('/', segments[..^1]);
            }
        ...
        string stackRoot, string patchKey, string category, string relativePath)
    {
        ...
        var dir = subfolder is null
            ? baseDir
            : System.IO.Path.Combine(baseDir, subfolder.Replace('/', System.IO.Path.DirectorySeparatorChar));
