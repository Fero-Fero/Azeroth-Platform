...
public sealed partial class MigrationService
{
    ...
    public async Task<ApplyPatchResultDto> ApplyPatchAsync(string stackId, string patchKey, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // 6.5) Pack every loose file/folder into the single archive named in mpq.json add
            //     (string or array). Existing .mpq files are published as-is, not packed.
            var manifest = ReadMpqManifest(stackRoot, patch.Key) ?? new MpqManifestDto();
            var archivesToBuild = MpqPackFilter.ConstructedArchiveNames(manifest);
            if (archivesToBuild.Count > 0)
            {
                await RunStageAsync("mpq-build", result, async () =>
                {
                    foreach (var mpqName in archivesToBuild)
                    {
                        ...
                        var mpqDir = MigrationLayout.MpqDir(stackRoot, patch.Key);
                        var mpqPath = Path.Combine(mpqDir, mpqName);
                        if (File.Exists(mpqPath) && !MpqPackFilter.HasPackableContentFor(mpqDir, mpqName))
                        {
                            AddLog(result, $"Pre-built {mpqName} found; publishing as-is (not reconstructed).");
                            continue;
                        }
            ...
            // 8) Run the patch's SQL LAST among content stages, once DBC/maps/MPQ have succeeded.
            //    SQL is the most failure-prone step (e.g. duplicate keys in third-party content), so
            //    keeping it here means a SQL error never leaves the client/data work half-applied.
            if (hasSql)
            {
            ...
            // Persist the applied level only after every content stage succeeded. Recording it
            // earlier (before news/rescan) left a phantom applied index on failure, which greys
            // out delete/drop after the operator drops and reimports the same patch folders.
            ...
            if (constructionPlan.ToBuild.Count > 0)
            {
                await RunStageAsync("mpq-build", result, async () =>
                {
                    ...
                    foreach (var entry in constructionPlan.ToBuild)
                    {
                        if (entry.PreBuilt)
                        {
                            AddLog(result, $"Pre-built {entry.MpqName} found in {entry.PatchKey}; publishing as-is (not reconstructed).");
                            continue;
                        }
        ...
        ApplyPatchResultDto result, CancellationToken cancellationToken)
    {
        ...
        foreach (var (subfolder, database) in MigrationLayout.SqlDatabases)
        {
            ...
            foreach (var sqlFile in sqlFiles)
            {
                ...
                // InnoDB rolls back the whole file. This protects only the DML that AC patch SQL uses:
                // DDL (CREATE/ALTER/DROP) auto-commits in MySQL and cannot be rolled back.
        ...
        IReadOnlyList<string> mpqRemovals, ApplyPatchResultDto result, CancellationToken cancellationToken)
    {
        ...
        if (Directory.Exists(overlayDataDir))
        {
            foreach (var leftover in Directory.EnumerateFiles(overlayDataDir))
            {
                var leftoverName = Path.GetFileName(leftover);
                if (!MpqPackFilter.IsWowContentFolderArchive(leftoverName))
                {
                    continue;
                }

                File.Delete(leftover);
                volumePathsToDelete.Add($"Data/{leftoverName}");
                AddLog(result, $"Removed leftover content-folder archive from overlay: {leftoverName}");
            }
        }
        ...
        {
            var name = Path.GetFileName(mpq);
            if (MpqPackFilter.IsWowContentFolderArchive(name)
                || SharedClientDataFiles.IsSharedBaseDataFile($"Data/{name}"))
            {
                AddLog(result, $"Skipping {name}; content-folder and stock archives are not published to the overlay.");
                continue;
            }

            var dest = Path.Combine(overlayDataDir, name);
        ...
        AddLog(result, $"Published {published} MPQ file(s) to the client overlay (overwriting same-named files).");
        await PushOverlayToEngineAsync(stack, stackRoot, result, cancellationToken);
    }

    /// <summary>
    /// Builds one MPQ from loose files and folders in the patch's mpq directory (mpq.json <c>add</c>).
    /// Existing <c>.mpq</c> archives, manifests, and sidecars are left out of the pack and published separately.
    /// Archive paths are the relative paths from the content tree (Interface\, World\, …), not a folder named after the MPQ.
    /// </summary>
        ...
        string mpqName, ApplyPatchResultDto result, CancellationToken cancellationToken)
    {
        ...
        var contentDir = MpqPackFilter.ContentDirectoryFor(mpqDir, mpqName);

        var workDir = Path.Combine(stackRoot, ".migration-tmp", $"mpqbuild-{Guid.NewGuid():N}");
        var stageDir = Path.Combine(workDir, MpqPackFilter.ConstructedArchiveStageFolderName);
        Directory.CreateDirectory(stageDir);

        try
        {
            foreach (var file in MpqPackFilter.EnumeratePackableFiles(contentDir))
            {
            ...
            var toolArgs = MpqPackFilter.ConstructedArchiveToolArgs(mpqName);
            var run = await _remoteEngine.RunToolWithWorkVolumeAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var publishedId = PatchNewsReader.SanitizePublishedId(article.Id);
        if (string.IsNullOrEmpty(publishedId))
        {
            AddLog(result, "Failed to publish patch news: article id is empty after sanitizing.");
            return false;
        }
        ...
        await RunStageAsync("patch-news", result, async () =>
        {
            var published = new LauncherNewsItemDto
            {
                ...
                IsDraft = false,
                Html = PatchNewsReader.RewriteHtmlForPublish(article.Html, stackId, publishedId),
            };

            await _launcherPortal.MergeStackNewsArticleAsync(stackId, published, cancellationToken);
            if (coverPath is not null)
            {
                await _launcherPortal.MergeStackNewsCoverFromFileAsync(stackId, publishedId, coverPath, cancellationToken);
            }
            ...
            foreach (var relativeAsset in PatchNewsReader.EnumerateAssetFiles(stackRoot, patchKey))
            {
                ...
                var assetId = PatchNewsReader.ToPublishedAssetId(publishedId, relativeAsset);
    ...
    /// Runs an authenticated <c>POST /rescan</c> from inside the container (via <c>docker exec</c>,
    /// using the container's own <c>CLIENT_AUTH_TOKEN</c>, context-aware for external stacks).
    /// </summary>
        ...
        Data.Entities.ManagedStackEntity stack, ApplyPatchResultDto result, CancellationToken cancellationToken)
    {
        ...
        // CLIENT_AUTH_TOKEN so a missing/mismatched DB ArmorySessionSecret cannot 401 the rescan.
        var args = ClientContainerService.BuildLoopbackMutatingCurlArgs(
            string.IsNullOrEmpty(_engineContext) ? null : _engineContext,
            container,
            "rescan",
            port);
