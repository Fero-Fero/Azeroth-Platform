...
public static class DockerComposeOverrideGenerator
{
    ...
    public static string? GetContainerNameForService(string stackId, string? stackName, string composeService)
    {
        var suffix = composeService switch
        {
            ...
            LlmChatterBridge.ServiceName => "-llm-chatter-bridge",
            _ => null,
        };
    ...
    /// <summary>
    /// Compose volume key for the manager's data volume when db-import bind-mounts a sanitized
    /// <c>modules</c> subpath (local stacks only). Declared <c>external: true</c> with the
    /// configured data volume name.
    /// </summary>
    public const string ManagerDataVolumeKey = "azp_manager_data";
    ...
    public const string OllamaService = "ollama";
    public const string OllamaPullService = "ollama-pull";
    public const string OllamaModelsVolumeKey = "ollama_models";

    /// <summary>
    /// Internal-only compose network for the Ollama serve sidecar. <c>internal: true</c> removes the
    /// default gateway, so the process cannot reach the internet (HTTPS, DNS, anything). Worldserver
    /// joins the same network to call <c>http://ollama:11434</c>.
    /// </summary>
    public const string OllamaNetworkKey = "ac-ollama";

    public static string OllamaInternalNetworkName(string stackId) => $"acore-{stackId}-ollama";

    private static readonly IReadOnlyDictionary<string, string> EmptyEnv =
        new Dictionary<string, string>();

    /// <summary>
    /// Lets AzerothCore create missing <c>acore_*</c> databases.
    ...
    /// <c>AC_CONSOLE_ENABLE=0</c> stops the world/auth <c>AC&gt;</c> CLI thread. A detached stdin
    /// either shuts the daemon down (EOF) or spams the prompt; SOAP is the management console.
    /// <c>AC_UPDATES_EXCEPTION_SHUTDOWN_DELAY</c> is required by current AzerothCore db-import even
    /// when <c>dbimport.conf</c> was seeded from an older dist that omitted the key.
    /// Worldserver and authserver images already set <c>AC_UPDATES_ENABLE_DATABASES=0</c> so only
    /// db-import applies SQL; this array must not override that to 7.
    /// </summary>
    private static readonly (string Key, string Value)[] AcDatabaseAutoSetupEnv =
    [
        ("AC_UPDATES_AUTO_SETUP", "1"),
        ("AC_UPDATES_EXCEPTION_SHUTDOWN_DELAY", "10000"),
        ("AC_DISABLE_INTERACTIVE", "1"),
        ("AC_CONSOLE_ENABLE", "0"),
    ];

    /// <summary>
    /// Path of <paramref name="localPath"/> relative to the manager data-volume mount (parent of
    /// <paramref name="buildsPath"/>). Used as a Compose volume <c>subpath</c> so db-import reads
    /// the live sanitized modules tree instead of the image COPY.
    /// </summary>
    public static bool TryGetDataVolumeSubpath(string localPath, string buildsPath, out string relative)
    {
        relative = string.Empty;
        if (string.IsNullOrWhiteSpace(localPath) || string.IsNullOrWhiteSpace(buildsPath))
        {
            return false;
        }
        ...
        var fullSource = Path.GetFullPath(localPath);
    ...
    public static string Generate(
        ...
        OllamaComposeOptions? ollama = null,
        string? managerDataVolumeName = null,
        string? modulesSubpath = null,
        LlmChatterBridgeComposeOptions? llmChatterBridge = null)
    {
        var containerPrefix = GetContainerPrefix(stackId, stackName);
        var useManagerModules = !string.IsNullOrWhiteSpace(managerDataVolumeName)
            && !string.IsNullOrWhiteSpace(modulesSubpath);
        ...
        AppendDbImportOverride(
            sb,
            stackId,
            $"{containerPrefix}-db-import",
            external,
            useManagerModules ? managerDataVolumeName : null,
            useManagerModules ? modulesSubpath : null);
        AppendWorldserverOverride(
            sb,
            stackId,
            containerPrefix,
            Bucket(serviceEnvironment, WorldserverService),
            includeLua,
            external,
            attachOllamaNetwork: ollama is not null);
        ...
        {
            AppendOllamaPullService(sb, $"{containerPrefix}-ollama-pull", ollama);
            AppendOllamaService(sb, $"{containerPrefix}-ollama", ollama);
        }

        if (llmChatterBridge is not null)
        {
            AppendLlmChatterBridgeService(
                sb,
                $"{containerPrefix}-llm-chatter-bridge",
                llmChatterBridge,
                attachOllamaNetwork: ollama is not null);
        }
        ...
        if (ollama is not null)
        {
            AppendExternalVolume(sb, ollama.ModelsVolumeKey, ollama.ModelsVolumeName);
        }

        if (useManagerModules)
        {
            AppendExternalVolume(sb, ManagerDataVolumeKey, managerDataVolumeName!);
        }
        ...
        if (ollama is not null)
        {
            ...
            sb.AppendLine($"  {OllamaNetworkKey}:");
            sb.AppendLine($"    name: {OllamaInternalNetworkName(stackId)}");
            sb.AppendLine("    driver: bridge");
            sb.AppendLine("    internal: true");
        }
    ...
    private static void AppendDbImportOverride(
        StringBuilder sb,
        string stackId,
        string containerName,
        bool external,
        string? managerDataVolumeName,
        string? modulesSubpath)
    {
        ...
        AppendEnv(sb, "AC_UPDATES_ENABLE_DATABASES", "7");
        ...
        // Pin the volume list so Compose cannot drop etc/logs, and so db-import reads the host
        // checkout (install hooks rewrite SQL that AC cannot apply) instead of the image COPY.
        // nocopy prevents Docker from populating an empty volume from that COPY. Worldserver/auth
        // keep image AC_UPDATES_ENABLE_DATABASES=0, so only this service applies SQL.
        sb.AppendLine("    volumes: !override");
        sb.AppendLine("      - ${DOCKER_VOL_ETC}:/azerothcore/env/dist/etc");
        sb.AppendLine("      - ${DOCKER_VOL_LOGS}:/azerothcore/env/dist/logs");
        sb.AppendLine("      - type: volume");
        if (!string.IsNullOrWhiteSpace(managerDataVolumeName) && !string.IsNullOrWhiteSpace(modulesSubpath))
        {
            sb.AppendLine($"        source: {ManagerDataVolumeKey}");
            sb.AppendLine("        target: /azerothcore/modules");
            sb.AppendLine("        volume:");
            sb.AppendLine("          nocopy: true");
            sb.AppendLine($"          subpath: {modulesSubpath}");
        }
        else
        {
            sb.AppendLine("        source: modules");
            sb.AppendLine("        target: /azerothcore/modules");
            sb.AppendLine("        volume:");
            sb.AppendLine("          nocopy: true");
        }
        ...
        bool attachOllamaNetwork)
    {
        ...
        // Do not depends_on ollama. SOAP and first boot must not wait for a model pull, and a later
        // `compose up -d` (or `up -d ollama`) would SIGTERM a worldserver that was started with --no-deps.
        ...
        EmitMergedEnv(
            ...
            {
                "AC_SOAP_ENABLED", "AC_SOAP_IP", "AC_SOAP_PORT", "AC_CONSOLE_ENABLE",
            });

        // Dual-home worldserver onto the internal Ollama network. Listing networks here replaces the
        // base compose list, so ac-network must be repeated or worldserver would lose MySQL/auth DNS.
        if (attachOllamaNetwork)
        {
            sb.AppendLine("    networks:");
            sb.AppendLine($"      - {AcNetworkName}");
            sb.AppendLine($"      - {OllamaNetworkKey}");
    ...
    /// <summary>
    /// One-shot model fetch on <see cref="AcNetworkName"/> (has a gateway). Isolated serve cannot
    /// reach the registry, so this job writes <see cref="OllamaComposeOptions.Model"/> into the
    /// shared models volume first. Profile-gated so <c>compose up -d</c> does not start it.
    /// </summary>
    private static void AppendOllamaPullService(StringBuilder sb, string containerName, OllamaComposeOptions ollama)
    {
        sb.AppendLine($"  {OllamaPullService}:");
        sb.AppendLine($"    image: {ollama.Image}");
        sb.AppendLine($"    container_name: {containerName}");
        sb.AppendLine("    restart: \"no\"");
        sb.AppendLine("    profiles:");
        sb.AppendLine($"      - {OllamaPullService}");
        sb.AppendLine("    volumes:");
        sb.AppendLine($"      - {ollama.ModelsVolumeKey}:/root/.ollama");
        AppendAcNetwork(sb);
        sb.AppendLine("    entrypoint: [\"/bin/sh\", \"-c\"]");
        sb.AppendLine("    command:");
        sb.AppendLine("      - |");
        sb.AppendLine("        ollama serve &");
        // Compose interpolates $VAR; $$ becomes a literal $. $$! → $! (background PID).
        sb.AppendLine("        pid=$$!");
        sb.AppendLine("        trap \"kill $$pid\" TERM INT");
        sb.AppendLine("        until ollama list >/dev/null 2>&1; do sleep 1; done");
        sb.AppendLine($"        ollama pull {ollama.Model}");
        sb.AppendLine("        kill $$pid");
    }

    /// <summary>
    /// Public Hub image (never <c>pull_policy: never</c>). Serve-only: no host ports, no gateway
    /// (<see cref="OllamaNetworkKey"/> is <c>internal: true</c>), cloud disabled. The model is
    /// pulled by <see cref="OllamaPullService"/>, then loaded into VRAM here and kept resident
    /// (<c>OLLAMA_KEEP_ALIVE=-1</c>) so bot chat is not waiting on a first-request warmup.
    /// </summary>
    private static void AppendOllamaService(StringBuilder sb, string containerName, OllamaComposeOptions ollama)
    {
        sb.AppendLine($"  {OllamaService}:");
        sb.AppendLine($"    image: {ollama.Image}");
        sb.AppendLine($"    container_name: {containerName}");
        sb.AppendLine("    restart: unless-stopped");
        sb.AppendLine("    environment:");
        sb.AppendLine("      OLLAMA_HOST: \"0.0.0.0\"");
        sb.AppendLine("      OLLAMA_NO_CLOUD: \"1\"");
        // Never unload: first bot chat must not pay a 15s+ GPU load (worldserver aborts that request).
        sb.AppendLine("      OLLAMA_KEEP_ALIVE: \"-1\"");
        // Short WoW replies; the Hub image otherwise picks ~4096 from VRAM and slows load/warmup.
        sb.AppendLine("      OLLAMA_CONTEXT_LENGTH: \"2048\"");
        sb.AppendLine("      OLLAMA_NUM_PARALLEL: \"1\"");
        switch (ollama.GpuBackend)
        {
            case GpuBackend.Nvidia:
                sb.AppendLine("      NVIDIA_VISIBLE_DEVICES: \"all\"");
                break;
            case GpuBackend.Vulkan:
                sb.AppendLine("      OLLAMA_VULKAN: \"1\"");
                break;
        }
        ...
        sb.AppendLine("        wait $$pid");
        sb.AppendLine("    healthcheck:");
        sb.AppendLine($"      test: [\"CMD-SHELL\", \"ollama ps | grep -F {ollama.Model}\"]");
        sb.AppendLine("      interval: 15s");
        sb.AppendLine("      timeout: 10s");
        sb.AppendLine("      retries: 120");
        sb.AppendLine("      start_period: 120s");
        AppendOllamaGpuDevices(sb, ollama.GpuBackend);
    }

    /// <summary>
    /// Device / reservation block for the probed backend. NVIDIA uses the compose deploy
    /// reservation; ROCm and Vulkan pass through host device nodes. CPU adds nothing.
    /// Docker Desktop/WSL2: NVIDIA works via the toolkit; <c>/dev/kfd</c> and <c>/dev/dri</c>
    /// exist only when WSL exposes them — otherwise the probe selects CPU.
    /// </summary>
    /// <summary>
    /// LLM Chatter's Python worker. The worldserver only queues rows for it, so it is not a
    /// <c>depends_on</c> of any game service; it needs the database (on <see cref="AcNetworkName"/>)
    /// and the model host (on <see cref="OllamaNetworkKey"/>). The image is manager-built per stack
    /// and never published, so pulling is disabled on every deployment target.
    /// </summary>
    private static void AppendLlmChatterBridgeService(
        StringBuilder sb,
        string containerName,
        LlmChatterBridgeComposeOptions bridge,
        bool attachOllamaNetwork)
    {
        sb.AppendLine($"  {LlmChatterBridge.ServiceName}:");
        sb.AppendLine($"    image: {bridge.ImageName}");
        sb.AppendLine($"    container_name: {containerName}");
        sb.AppendLine("    restart: unless-stopped");
        AppendNeverPullPolicy(sb);
        // The bridge does not create its own tables; AzerothCore imports the module schema. Starting
        // before db-import finishes leaves it failing its health check against a table-less database.
        sb.AppendLine("    depends_on:");
        sb.AppendLine($"      {InternalDatabaseHost}:");
        sb.AppendLine("        condition: service_healthy");
        sb.AppendLine("      ac-db-import:");
        sb.AppendLine("        condition: service_completed_successfully");
        sb.AppendLine("    networks:");
        sb.AppendLine($"      - {AcNetworkName}");
        if (attachOllamaNetwork)
        {
        ...
        sb.AppendLine("    volumes:");
        sb.AppendLine($"      - ${{DOCKER_VOL_ETC}}:{LlmChatterBridge.ConfMountPath}:ro");
    }

    private static void AppendOllamaGpuDevices(StringBuilder sb, GpuBackend backend)
    {
        switch (backend)
        {
            case GpuBackend.Nvidia:
                ...
                sb.AppendLine("              count: all");
                sb.AppendLine("              capabilities: [gpu]");
                break;
            case GpuBackend.Rocm:
                sb.AppendLine("    devices:");
                sb.AppendLine("      - /dev/kfd");
                sb.AppendLine("      - /dev/dri");
                sb.AppendLine("    group_add:");
                sb.AppendLine("      - video");
...
/// <summary>
/// Values needed to render the Ollama sidecar. Image is always pulled from Docker Hub
/// (no <c>pull_policy: never</c>), including on external stacks. Serve is isolated on an
/// internal Docker network; the model is fetched by a one-shot puller that still has a gateway.
/// </summary>
public sealed record OllamaComposeOptions
{
    public required string Image { get; init; }

    public required string Model { get; init; }

    public required string ModelsVolumeName { get; init; }

    public required string ModelsVolumeKey { get; init; }

    public required GpuBackend GpuBackend { get; init; }

    public static OllamaComposeOptions FromSidecar(ModuleRuntimeSidecar sidecar, GpuBackend gpuBackend)
    {
        var requested = string.IsNullOrWhiteSpace(sidecar.Image) ? OllamaSidecar.Image : sidecar.Image;
        return new()
        {
            Image = OllamaSidecar.ResolveImage(requested, gpuBackend),
            Model = string.IsNullOrWhiteSpace(sidecar.Model) ? OllamaSidecar.Model : sidecar.Model,
            ModelsVolumeName = string.IsNullOrWhiteSpace(sidecar.ModelsVolumeName)
                ? OllamaSidecar.ModelsVolumeName
                : sidecar.ModelsVolumeName,
            ModelsVolumeKey = string.IsNullOrWhiteSpace(sidecar.ModelsVolumeKey)
                ? OllamaSidecar.ModelsVolumeKey
                : sidecar.ModelsVolumeKey,
...
/// <summary>
/// Values needed to render the LLM Chatter bridge. The image is built by the manager from the
/// module checkout and shipped to external engines, so it is never pulled.
/// </summary>
public sealed record LlmChatterBridgeComposeOptions
{
    public required string ImageName { get; init; }

    public static LlmChatterBridgeComposeOptions ForStack(string stackId) =>
        new() { ImageName = LlmChatterBridge.ImageTag(stackId) };
}
