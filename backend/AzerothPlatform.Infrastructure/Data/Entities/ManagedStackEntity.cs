...
public class ManagedStackEntity
{
    ...
    /// <summary>Per-stack git branch overrides keyed by catalog module id.</summary>
    public string ModuleBranchesJson { get; set; } = "{}";

    /// <summary>SHA-256 fingerprint of core+module SHAs from the last successful module compile check.</summary>
    public string ModuleCheckFingerprint { get; set; } = string.Empty;

    /// <summary>Serialized <see cref="ModuleCheckStatusDto"/> from the last module compile check.</summary>
    public string ModuleCheckJson { get; set; } = string.Empty;
    ...
    /// <summary>Checkpoint so a failed Express Setup retries from the step that broke.</summary>
    public ExpressProvisionPhase ExpressProvisionPhase { get; set; } = ExpressProvisionPhase.None;

    /// <summary>True after Express created game account admin / admin (GM 3).</summary>
    public bool ExpressGameAccountCreated { get; set; }

    /// <summary>One-time Overview notice: all ready, press Start.</summary>
    public bool ExpressReadyNoticePending { get; set; }
    ...
    /// <summary>
    /// SOAP console credentials, generated per stack at creation. Blank rather than a well-known
    /// default, so a creation path that forgets to assign them fails authentication instead of
    /// silently granting remote console access under a guessable account.
    /// </summary>
    ...
    public bool LauncherVisible { get; set; } = true;
