...
public sealed class PatchNewsReaderTests
{
    ...
    public void ToPublishedAssetId_encodes_relative_path()
    {
        PatchNewsReader.ToPublishedAssetId("progression-2-0", "images/karazhan.png")
            .Should().Be("progression-2-0-images-karazhan");
    }

    [Fact]
    public void ToPublishedAssetId_strips_extension_and_unsafe_characters()
    {
        PatchNewsReader.ToPublishedAssetId("progression-1.0-classic", "images/Molten Core.png")
            .Should().Be("progression-1-0-classic-images-Molten-Core");
    }

    [Fact]
    public void SanitizePublishedId_keeps_safe_article_ids()
    {
        PatchNewsReader.SanitizePublishedId("progression-1-0-classic")
            .Should().Be("progression-1-0-classic");
    }

    [Fact]
    public void RewriteHtmlForPublish_uses_sanitized_asset_ids()
    {
        const string html = """<img src="images/karazhan.png" alt="" />""";
        var rewritten = PatchNewsReader.RewriteHtmlForPublish(html, "stack-1", "progression-2-0");

        rewritten.Should().Contain("/api/stacks/stack-1/launcher/news-image/progression-2-0-images-karazhan");
        rewritten.Should().NotContain("karazhan.png");
    }
