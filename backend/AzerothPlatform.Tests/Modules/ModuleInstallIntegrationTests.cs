...
/// MPQs are zip archives, so CI never runs WDBX, mpqtool, or client-data.
/// </summary>
public sealed class ModuleInstallIntegrationTests
{
    ...
    [Fact]
    public async Task Apply_exports_baseline_csv_from_server_dbc_when_store_is_empty()
    {
        using var fx = new Fixture();
        SeedServerDbc(fx, "SkillLine.dbc", VanillaSkillLine);
        WriteDeltaModule(fx, "mod-alpha", "SkillLine.dbc", VanillaSkillLine + "90001,Alpha\r\n");
        ...
        fx.Store.FindTableCsv("SkillLine").Should().NotBeNull();
        var imported = await File.ReadAllTextAsync(Path.Combine(fx.StackRoot, "server_dbc", "SkillLine.dbc"));
        imported.Should().Contain("1,Vanilla");
        imported.Should().Contain("90001,Alpha");
        var delta = await File.ReadAllTextAsync(
            Path.Combine(fx.StackRoot, "InstalledModules", "mod-alpha", "csv", "SkillLine.txt"));
        delta.Should().Contain("90001,Alpha");
        delta.Should().NotContain("1,Vanilla");
    }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        public (ModuleInstallSession Session, ModuleInstallHelpers Helpers) Helpers(string moduleId, string packageRoot)
        {
            ...
            var helpers = new ModuleInstallHelpers(
                moduleId, packageRoot, session, Wdbx, Store, Mpq, MigrationLayout.ServerDbcDir(StackRoot));
    ...
    private sealed class FileDbcStore : IDbcBaselineStore
    {
        ...
        {
            var existing = FindTableCsv(tableName);
            ...
            var dest = Path.Combine(Dir, CsvNormalizer.TableFileName(tableName));
            Directory.CreateDirectory(Dir);
