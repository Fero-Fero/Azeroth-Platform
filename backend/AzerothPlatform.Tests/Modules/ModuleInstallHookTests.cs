...
public sealed class ModuleInstallSessionTests
{
    ...
    {
        using var root = new TempDir("azp-session");
        using var session = new ModuleInstallSession(root.Path);
        session.SetBaseDbc(new SessionBaseDbc
        {
            TableName = "Spell",
            ModuleId = "mod-foo",
            BinaryPath = root.Combine("Spell.dbc")
        });
        ...
        var act = () => session.SetBaseDbc(new SessionBaseDbc
        {
            TableName = "Item",
            ModuleId = "mod-bar",
            BinaryPath = root.Combine("Item.dbc")
        });
    ...
    {
        using var root = new TempDir("azp-session");
        var session = new ModuleInstallSession(root.Path);
        ...
        Directory.Exists(root.Path).Should().BeTrue();
        Directory.Exists(extracted).Should().BeFalse();
        File.Exists(root.Combine("mod-foo", "csv", "Spell.txt")).Should().BeTrue();
    }

    [Fact]
    public void Dispose_keeps_extracted_when_failed_and_keepOnFailure()
    {
        using var root = new TempDir("azp-session");
        var session = new ModuleInstallSession(root.Path, keepOnFailure: true);
        var extracted = Path.Combine(session.ModuleDir("mod-foo"), "extracted");
        Directory.CreateDirectory(extracted);
        File.WriteAllText(Path.Combine(extracted, "scratch.txt"), "x");
        session.MarkFailed();
        session.Dispose();
        File.Exists(Path.Combine(extracted, "scratch.txt")).Should().BeTrue();
    }
}
...
public sealed class ModuleInstallHookRunnerTests
{
    ...
    [Fact]
    public void PrepareCheckouts_invokes_matching_hook_only()
    {
        var root = Path.Combine(Path.GetTempPath(), "azp-hooks-" + Guid.NewGuid().ToString("N"));
        var simDir = Path.Combine(root, PlayerbotDungeonSimInstallHook.CatalogId, "sql");
        Directory.CreateDirectory(simDir);
        var sql = Path.Combine(simDir, "proc.sql");
        File.WriteAllText(sql, "DELIMITER $$\nCREATE PROCEDURE `x`() BEGIN SELECT 1; END$$\nDELIMITER ;\n");
        try
        {
            var runner = new ModuleInstallHookRunner(
                [new FakeHook("mod-foo"), new PlayerbotDungeonSimInstallHook()]);
            runner.PrepareCheckouts(root).Should().ContainSingle();
            File.ReadAllText(sql).Should().Be(PlayerbotDungeonSimInstallHook.SilentStub);
        }
        finally
        {
