...
public sealed class ServerTypeRequiredModuleTests
{
    ...
    public void GetServerTypes_exposes_express_as_local_only_with_required_modules()
    {
        ...
        type.RequiredModuleIds.Should().Contain("mod-ale");
        type.RequiredModuleIds.Should().NotContain("mod-ollama-bot-buddy");
        type.RequiredModuleIds.Should().NotContain("mod-ollama-chat");
        type.CoreRepositoryUrl.Should().Contain("Grimfeather");
    }

    [Fact]
    public async Task ValidateAsync_accepts_express_without_ollama_module()
    {
        await using var db = CreateDbContext();
        var modules = CreateModuleCatalogMock();
        var serverTypes = CreateServerTypeCatalogMock();
        ...
        var configuration = CreateBaseConfiguration(ServerType.Express, ["mod-individual-progression"]);

        var result = await validator.ValidateAsync(configuration);

        result.Errors.Should().NotContain(error =>
            error.Message.Contains("Ollama", StringComparison.OrdinalIgnoreCase));
    }
