...
public sealed class PatchDeleteTests
{
    ...
    [Fact]
    public async Task DeleteAllPatchesAsync_resets_applied_level_so_reimport_is_not_locked()
    {
        ...
        try
        {
            ...
            await using var db = CreateDbContext(new ManagedStackEntity
            {
                ...
                AppliedPatchesJson = """[{"key":"patch 1.0","level":1000000}]""",
                ModuleIdsJson = "[]",
            });
            var service = CreateMigrationService(db, buildsPath);
            ...
            deleted.Should().BeGreaterThan(0);
            Directory.EnumerateDirectories(MigrationLayout.MigrationsRoot(stackRoot)).Should().BeEmpty();
            var stack = await db.ManagedStacks.SingleAsync(item => item.Id == stackId);
            stack.AppliedPatchLevel.Should().Be(0);
            stack.AppliedPatchesJson.Should().Be("[]");
        }
        finally
        {
            if (Directory.Exists(buildsPath))
