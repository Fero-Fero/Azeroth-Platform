...
using AzerothPlatform.Core.Serialization;
...
/// tries to upload or apply a DBC file that is not allowed (binary .dbc always; CSV on Express only).
/// </summary>
...
{
    /// <summary>
    /// Archive name(s) to construct from loose files and folders in this patch's mpq directory.
    /// Progression manifests use a single string (<c>"add": "Patch-W.MPQ"</c>); arrays are also accepted.
    /// </summary>
    [JsonConverter(typeof(JsonStringOrStringListConverter))]
