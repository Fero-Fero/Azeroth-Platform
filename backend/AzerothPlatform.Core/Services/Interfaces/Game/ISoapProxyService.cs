...
public interface ISoapProxyService
{
    ...
    /// <summary>
    /// True when the worldserver container is running and its SOAP port accepts connections.
    /// Does not log connection failures as errors — used to wait quietly before issuing commands.
    /// </summary>
    Task<bool> IsReachableAsync(string stackId, CancellationToken cancellationToken = default);
}
