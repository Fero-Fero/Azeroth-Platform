...
/// <summary>
/// Per-module recipe keyed by catalog id: extra-data install plus optional compile/checkout.
/// Missing hook = C++ / conf / data/sql only, cloned into a folder named after the catalog id.
/// </summary>
public interface IModuleInstallHook
{
    string ModuleId { get; }

    /// <summary>Compile/checkout exceptions. Default is an empty profile (folder = catalog id).</summary>
    ModuleCompileProfile Compile => ModuleCompileProfile.Empty;
    ...
    /// <summary>
    /// After git clone/reset, rewrite files in this module's checkout before CMake or db-import.
    /// Returns the paths that changed. Default is a no-op.
    /// </summary>
    IReadOnlyList<string> PrepareCheckout(string moduleDir) => [];
}
...
public interface IModuleInstallHookRunner
{
    ...
    /// <summary>
    /// Runs <see cref="IModuleInstallHook.PrepareCheckout"/> for every hook whose checkout exists
    /// under <paramref name="modulesPath"/>. Returns the paths that changed.
    /// </summary>
    IReadOnlyList<string> PrepareCheckouts(string modulesPath);
}
