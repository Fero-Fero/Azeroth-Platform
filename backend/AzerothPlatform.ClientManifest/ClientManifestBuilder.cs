...
// The assembly is AzerothPlatform.ClientManifest. This namespace cannot be that name:
// AzerothPlatform.ClientManifest would hide Core.Contracts.ClientManifest in every
// AzerothPlatform.* compilation unit.
...
public sealed class ClientManifestResult
{
    ...
    /// <summary>
    /// Fingerprint of the on-disk layout this manifest was built from (see
    /// <see cref="ClientManifestBuilder.ComputeShapeSignature"/>). Recomputing it is cheap, so a server
    /// can tell whether its cached manifest still matches reality without rehashing anything.
    /// </summary>
...
public static class ClientManifestBuilder
{
    ...
    /// <summary>
    /// Marker the client container stamps so launchers can be forced through a one-off full re-hash.
    /// Lives beside the cache files and is bookkeeping rather than served content.
    /// </summary>
    public const string VerifyTokenFileName = ".verifytoken";

    /// <summary>
    /// Everything the cache directory holds that describes the current content, as opposed to what the
    /// stack itself is (portal registry, branding, news). Clearing exactly these rebuilds the manifest
    /// from the files on disk.
    /// </summary>
    public static readonly IReadOnlyList<string> CacheBookkeepingFiles =
        [HashCacheFileName, ManifestFileName, VerifyTokenFileName];
        ...
        string? signingPrivateKey = null)
    {
        ...
        var resolved = ResolveContentFiles(gameRoots);
        var shapeEntries = new List<string>(resolved.Count);
        ...
        foreach (var (relativePath, absolutePath) in resolved)
        {
            ...
            if (!info.Exists)
            {
                // Deleted between enumeration and stat (a concurrent volume seed). Skip it rather than
                // failing the whole scan; the shape signature will not match and we rebuild shortly.
                continue;
            }

            var mtimeTicks = info.LastWriteTimeUtc.Ticks;
            shapeEntries.Add(FormatShapeEntry(relativePath, info.Length, mtimeTicks));
        ...
        return new ClientManifestResult
        {
            Manifest = manifest,
            Files = resolved,
            ShapeSignature = HashShapeEntries(shapeEntries)
        };
    }

    /// <summary>
    /// Resolves the manifest path set across the given roots, applying the exclusion rules and the
    /// merge policy. Shared by <see cref="BuildAsync"/> and <see cref="ComputeShapeSignature"/> so a
    /// scan and the change detection guarding it can never disagree about which files count.
    ///
    /// Merge policy: paths present in only one root are simply served from it, so an uploaded base
    /// client and the patch MPQs a server-wide progression published sit side by side untouched. When
    /// the same path exists in more than one root the newest file wins, so re-uploading a client does
    /// not silently revert a newer published patch, and republishing a patch does not lose to a stale
    /// base copy. Ties go to the later root (the overlay), which is where managed content belongs.
    ///
    /// The one exception is the stock Blizzard archives: those are shared base content and the overlay
    /// must never shadow them regardless of timestamps, or a stray file in the overlay would replace
    /// common.mpq for every player.
    /// </summary>
    public static Dictionary<string, string> ResolveContentFiles(IReadOnlyList<string> gameRoots)
    {
        var resolved = new Dictionary<string, string>(StringComparer.Ordinal);
        var overlayRoot = false;
        ...
        foreach (var root in gameRoots)
        {
            ...
            foreach (var absolutePath in Directory.EnumerateFiles(root, "*", SearchOption.AllDirectories))
            {
                ...
                if (IsExcludedFromManifest(relativePath))
                {
                    continue;
                }

                if (overlayRoot && SharedClientDataFiles.MustNotServeFromOverlay(relativePath))
                {
                    continue;
                }

                if (resolved.TryGetValue(relativePath, out var incumbent)
                    && !IsAtLeastAsRecent(absolutePath, incumbent))
                {
                    continue;
                }
    ...
    /// <summary>
    /// True when <paramref name="candidate"/> is no older than <paramref name="incumbent"/>. Equal
    /// timestamps resolve in the candidate's favour so the later root wins a tie. A file we cannot stat
    /// loses, since serving the one we can read is the safer outcome.
    /// </summary>
    private static bool IsAtLeastAsRecent(string candidate, string incumbent)
    {
        try
        {
    ...
    /// <summary>
    /// Cheap fingerprint of what is currently on disk: every manifest path with its size and mtime,
    /// hashed. No file contents are read, so this is a plain directory walk even for a multi-GB client.
    /// A server compares this against the signature of its cached manifest to notice content changes
    /// nobody told it about — a rescan poke that failed, or a file written straight into the volume.
    /// </summary>
    public static string ComputeShapeSignature(IReadOnlyList<string> gameRoots)
    {
        var resolved = ResolveContentFiles(gameRoots);
        var entries = new List<string>(resolved.Count);
        ...
        foreach (var (relativePath, absolutePath) in resolved)
        {
            ...
            if (info.Exists)
            {
                entries.Add(FormatShapeEntry(relativePath, info.Length, info.LastWriteTimeUtc.Ticks));
            }
        }

        return HashShapeEntries(entries);
    }

    private static string FormatShapeEntry(string relativePath, long size, long mtimeTicks) =>
        $"{relativePath}:{size}:{mtimeTicks}";

    private static string HashShapeEntries(List<string> entries)
    {
    ...
    /// <summary>
    /// Files that are never distributed: our own bookkeeping, and the client's <c>WTF/</c> tree which
    /// holds per-player runtime state (Config.wtf, per-account SavedVariables). Server-owned values
    /// reach the client via the rendered Config.wtf settings template the launcher merges in.
    /// </summary>
    private static bool IsExcludedFromManifest(string relativePath) =>
        relativePath is HashCacheFileName or ManifestFileName || IsPlayerRuntimeState(relativePath);
