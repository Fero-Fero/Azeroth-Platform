...
/// <summary>
/// Signs and verifies <see cref="ClientManifest"/> instances with ECDSA (P-256 / SHA-256). This is the
...
public static class ManifestSigner
{
    ...
    /// itself. Both signer and verifier must iterate <see cref="ClientManifest.Files"/> in the same
    /// order (the builder sorts by <c>RelativePath</c> and that order is preserved through JSON).
    /// </summary>
    public static byte[] CanonicalBytes(ClientManifest manifest)
    {
    ...
    /// <summary>
    /// Signs the manifest in place, setting <see cref="ClientManifest.Signature"/>. No-op when
    /// <paramref name="privateKeyPkcs8Base64"/> is blank (signing disabled).
    /// </summary>
    public static void Sign(ClientManifest manifest, string? privateKeyPkcs8Base64)
    {
