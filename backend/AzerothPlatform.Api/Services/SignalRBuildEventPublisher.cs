...
public class SignalRBuildEventPublisher : IBuildEventPublisher
{
    ...
    public Task PublishModuleCheckUpdatedAsync(string stackId, IReadOnlyList<ModuleCheckItemDto> items)
    {
        return _hubContext.Clients.Group(stackId).SendAsync("ModuleCheckUpdated", stackId, items);
    }
}
