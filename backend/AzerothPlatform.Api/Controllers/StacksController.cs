...
public class StacksController : ControllerBase
{
    ...
    [HttpPost("{stackId}/modules/sync")]
    public async Task<ActionResult<SyncStackModulesResultDto>> SyncStackModules(
        string stackId,
        [FromBody] SyncStackModulesRequestDto? request,
        CancellationToken cancellationToken)
    {
        try
        {
            var result = await _buildService.SyncModulesAsync(stackId, request?.ModuleId, cancellationToken);
            return Ok(result);
        }
        catch (KeyNotFoundException)
        {
    ...
    [HttpPost("{stackId}/modules/check")]
    public async Task<ActionResult<BuildStartedResponse>> CheckModules(
        string stackId,
        CancellationToken cancellationToken)
    {
        try
        {
            var buildStatus = await _buildService.CheckModulesAsync(stackId, cancellationToken);
    ...
    public async Task<ActionResult<BuildStartedResponse>> StartBuild(
        ...
        [FromQuery] bool skipModuleCheck = false)
    {
        var stack = await _stackService.GetAsync(stackId, cancellationToken);
        if (stack is null)
        {
        ...
        try
        {
            var buildStatus = await _buildService.StartAsync(stackId, buildConfig, cancellationToken, skipModuleCheck);
            return Ok(new BuildStartedResponse
            {
                BuildId = buildStatus.BuildId,
                Status = buildStatus.CurrentPhase.ToString()
    ...
    /// standard stack detail payload withholds the root password.
    /// </summary>
