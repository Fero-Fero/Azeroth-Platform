# Azeroth Platform - Technical Reference

Companion to the high-level **[README](./README.md)**. This document covers architecture, install,
the first-tour walkthrough, configuration, development, troubleshooting, and a full feature
reference.
...
Azeroth Platform is a web-based control panel for private World of Warcraft 3.3.5a (Wrath of the
Lich King) servers built on [AzerothCore](https://www.azerothcore.org/). It deploys, builds,
configures, and manages your servers in Docker - and it distributes the game client and a branded
launcher to your players - all from one dashboard.
...
- 🤖 **AI bot chat** - pick one of Bot Buddy, Chat, or LLM Chatter; the stack runs a shared Ollama
  sidecar (and a Python bridge for LLM Chatter), downloads the model on first start, and turns
  Playerbots on automatically.
...
- Stacks that select an AI chat module also attach the shared `azeroth-platform-ollama-models`
  volume (the LLM weights; a multi-gigabyte download on first start).
...
**Remote host requirements:** Ubuntu or Debian, reachable over SSH from the manager, Docker installed,
and the SSH user able to run it (a non-root user in the `docker` group). Configure VPC security roles in the
stack wizard (Deployment step) and sync the host firewall from the stack VPC overview tab after
deploy. Optionally apply cloud security group rules from the same panel when using a linked account.
...
4. Launch downloads a first-login `.pem` (`ubuntu.pem` / `root.pem`) and **`azp-admin.pem`** (daily SSH). Verify **`azp-admin.pem`**, then **Verify VPC**. Wait 1–2 minutes after launch so user-data can finish. Verify VPC SSHs as ubuntu/root with the bootstrap key, locks those accounts, then confirms azp-admin SSH.
5. Continue the wizard. Unfinished stacks stay as **Finish setup** drafts.
...
The Deployment step is a four-stage flow for **Linux** (Ubuntu/Debian) external stacks. AzerothCore
runs as **Linux Docker containers**. Supported cloud providers: **DigitalOcean, Hetzner, Vultr, AWS,
GCP, and Azure**.

| Step | What you do |
| --- | --- |
| **1 - OS** | VPC hosts must be **Ubuntu or Debian**. Windows Server is not supported. |
| **2 - Connect** | Link a cloud account, pick an existing VM or launch a new one, or enter a host and SSH key. |
| **3 - Verify certificate** | Launch downloads two files named after the SSH users: **`ubuntu.pem` or `root.pem`**, and **`azp-admin.pem`**. Verify **`azp-admin.pem`**. |
| **4 - Verify VPC** | SSH as ubuntu/root with the manager key, create/lock azp-admin, disable internet SSH for ubuntu/root, then confirm azp-admin SSH, Docker, ufw, and the cloud firewall. Resume skips ubuntu/root after the Linux lock. |
...
Launch generates two keys and downloads both, named after the SSH users: **`ubuntu.pem` or `root.pem`**
(first login) and **`azp-admin.pem`** (daily SSH). Verify azp-admin.pem. After Verify VPC locks ubuntu/root, daily SSH is azp-admin;
break-glass is the provider console (AWS: Instance Connect as `ubuntu`). Wait 1–2 minutes after create before Verify VPC
(user-data is async). SSH on the cloud firewall is pinned to your admin IP when known; if you change networks,
use **Update SSH to my IP** on the stack VPC panel.
...
client 8101; never MySQL 3306 or SOAP 7878). Verify VPC probes that firewall. Break-glass after Verify VPC
locks ubuntu/root is **Hetzner KVM Console**. Optional default **location** (e.g. `nbg1`) limits instance listing.
...
in the target region; the platform creates security group `azeroth-platform-launch` (SSH from your
admin IP when known, plus auth 3724, world 8085, armory 8100, client 8101; never MySQL 3306 or SOAP
7878) if needed:
...
tag and those firewall rules. Break-glass after Verify VPC locks ubuntu/root is serial console or IAP.
...
create the Linux VM in Azure Portal first, then pick it. Break-glass after Verify VPC locks ubuntu/root is Bastion,
...
Scratch files the manager creates while it works (client archives, MPQ extracts, DBC exports, armory
image staging, addon uploads) all land under `%TEMP%/azeroth-platform` (or `$TMPDIR/azeroth-platform`
on Linux). `TempWorkspace` deletes each one when the operation finishes; `TempWorkspaceSweeper` then
collects any orphans older than six hours on an hourly pass, so a killed process cannot leak
scratch permanently.
...
#### AI bot chat
- **What:** three mutually exclusive modules that give playerbots an LLM to talk with — **Bot Buddy**
  (`mod-ollama-bot-buddy`), **Chat** (`mod-ollama-chat`), and **LLM Chatter** (`mod-llm-chatter`).
  Only one may be selected; they share a single **AI Bot Chatting** group in Express Setup and in
  the module catalog.
- **Where:** stack wizard → **Modules**, or stack → **Game** group → **Modules**. Hidden on server
  types that cannot satisfy the Playerbots dependency (for example NpcBots).
- **Beginner:** pick one. Playerbots is toggled on automatically (and off again if you deselect all
  three). The stack starts its own Ollama container, pulls `llama3.2:1b` on first start (a
  multi-gigabyte download), and writes `thinking=false` so the model does not emit chain-of-thought
  tokens into chat.
- **Advanced:** Bot Buddy and Chat disable the built-in playerbot chatter (`AiPlayerbot.*` broadcast
  / random-talk keys) so the two voices do not overlap. LLM Chatter layers on top of that chatter
  instead, and talks to Ollama through a managed Python bridge container (`llm-chatter-bridge`)
  rather than from the worldserver process. All three rewrite their Ollama URL to the internal
  sidecar (`http://ollama:11434`). See [DOCKER.md](./DOCKER.md#ai-chat-sidecars).
...
Architecture, React Query), write Conventional-Commits messages, and update docs when changing
APIs/features.

See **[CONTRIBUTING.md](./CONTRIBUTING.md)** for the conventions that are easy to get wrong — notably
the comment convention (`.cursor/rules/comments.mdc`) and the launcher/backend duplicated contracts —
and `.github/copilot-instructions.md` for coding standards.

When reporting bugs, use the [bug report form](https://github.com/Fero-Fero/AzerothPlatform/issues/new?template=bug_report.yml).
