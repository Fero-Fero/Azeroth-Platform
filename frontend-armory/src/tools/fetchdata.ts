...
async function download(dir: string, file: string): Promise<any> {
	...
	if (res.headers.get("Content-Type") === "application/json") {
		...
		const body = res.body;
		if (body === null) {
			throw new Error(`Empty response body for ${fullPath}`);
		}
...
function queueTexturesAndModels(item: any): void {
	if (item.TextureFiles !== null) {
		for (const file of Object.values<any>(item.TextureFiles)) {
	...
	if (item.ModelFiles !== null) {
		for (const file of Object.values<any>(item.ModelFiles)) {
			if (file["FileDataId"] !== 0) {
...
async function downloadRaces(): Promise<void> {
	...
	await promisepool.PromisePool.for(raceGenderCombo)
		...
		.process(async (race) => {
			...
			const textureFiles = Object.values<any>(customizationJson["TextureFiles"]).flat();
...
async function parseModels(): Promise<void> {
	const files = await globAsync("static/data/mo3/*.mo3");
