...
export interface IColumnSettings {
	name: string;
	collation?: string;
	formatter?: (data: any, row: any) => string | number | boolean;
	table?: string;
	database?: string;
}
...
export class DataTablesSsp {
	...
	public constructor(query: Query, db: Pool, table: string, primaryKey: string, columnSettings: IColumnSettings[]) {
		...
		const searchQuery = query.search as { value: string; regex: string };
		this.search = {
			value: searchQuery.value,
			regex: searchQuery.regex === "true",
		};
	...
	public async run(queryTimeout = 10_000): Promise<IResult> {
		...
		const recordsTotal = totalResult[0][0].count;
		const recordsFiltered = filteredResult[0][0].count;

		let rows = dataResult[0];
		rows = rows.map((row) => {
