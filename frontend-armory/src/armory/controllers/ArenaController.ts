...
import { IEmblem, IEmblemSource, Utils } from "../Utils";
...
export class ArenaController {
	...
	private async getTeamData(realm: IRealmConfig, teamName: string): Promise<ITeamData | null> {
		const db = this.armory.getCharactersDb(realm.name);
		...
		return {
			...
			emblem: Utils.makeEmblemObject(team as unknown as IEmblemSource, false),
			members,
		};
	}
}
