...
One WoW install (`%PROGRAMDATA%/{AppName}`) is shared across profiles. The base client downloads
...
with in transit). The install folder defaults to **`%PROGRAMDATA%\Azeroth Platform`** on Windows;
friends can change it in the Settings tab.

### Install folder and administrator rights

The client installs machine-wide so every Windows account on the PC shares one copy. That folder needs
an ACL only an administrator can set, so **Save** in the Settings tab raises a UAC prompt **once per
folder**: a short-lived elevated copy of the launcher grants `BUILTIN\Users` modify rights and exits.

Everything after that runs unelevated. This matters — an elevated `Wow.exe` resolves `Data\` from
`System32` and silently stops loading letter patches such as `patch-W`, so **Play** drops back to the
unelevated Explorer token even if the launcher itself happens to be elevated.

If the prompt is declined the launcher keeps working:

- when the folder is writable by the current account, it is used as a per-account install, and Save
  offers the grant again next time;
- when it is not writable at all, the launcher records that it must start elevated from then on.

Set `defaultInstallDirectory` in `launcher.settings.json` to pin a specific absolute path instead.
...
1. **Pre-set the download URL.** Copy [`AzerothPlatform.Launcher/launcher.settings.example.json`](AzerothPlatform.Launcher/launcher.settings.example.json)
   ...
   By default the install folder is `%PROGRAMDATA%\Azeroth Platform`; set
   `defaultInstallSubfolder` to change the folder name under `%PROGRAMDATA%`, or
