...
/// <summary>
/// The manager carries its own copy of this rule in
/// <c>backend/AzerothPlatform.ClientManifest/SharedClientDataFiles.cs</c>, and the two must agree: the
/// manager decides which archives never move to the overlay, the launcher decides which never get
...
/// this suite or its manager twin (<c>AzerothPlatform.Tests.Client.SharedClientDataFilesDriftTests</c>).
/// </summary>
public sealed class SharedClientDataFilesDriftTests
{
    ...
    private static IEnumerable<string> Probes()
    {
    ...
    public void Every_probe_is_classified_the_same_way_on_both_sides()
    {
        ...
        {
            var shared = Canonical.Contains(name, StringComparer.OrdinalIgnoreCase);

            SharedClientDataFiles.IsSharedBaseDataFile($"Data/{name}").Should().Be(shared, name);
            SharedClientDataFiles.IsProfileOverlayMpq(name).Should().Be(!shared, name);
        }
    }
