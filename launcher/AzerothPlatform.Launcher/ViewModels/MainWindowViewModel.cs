...
public partial class MainWindowViewModel : ObservableObject
{
    ...
    // Baked-in manifest signing pubkey. Manifests arrive over the stack's plain-HTTP channel, so this
    // key is the only thing their trust is anchored to.
    ...
    public MainWindowViewModel(LauncherStateStore stateStore)
    {
        ...
        _installDirectory = EnsureWritableInstallDirectory(_installDirectory, persist: _state.IsConfigured);
        _elevateOnStart = _state.ElevateOnStart;
    }

    /// <summary>
    /// Whether the launcher relaunches itself elevated on every start. Set automatically when the
    /// all-users grant fails, and exposed here so a player who declined the prompt once is not stuck
    /// approving UAC forever. Takes effect on the next start.
    /// </summary>
    [ObservableProperty]
    private bool _elevateOnStart;

    /// <summary>Elevation only exists on Windows; every other platform hides the setting.</summary>
    public static bool ShowElevationSetting => OperatingSystem.IsWindows();

    partial void OnElevateOnStartChanged(bool value)
    {
        if (_state.ElevateOnStart == value)
        {
            return;
        }

        _state.ElevateOnStart = value;
        _stateStore.Save(_state);
    }

    private string EnsureWritableInstallDirectory(string requested, bool persist)
    {
        if (InstallPathAccess.IsWritable(requested))
        {
            return requested;
        }

        var fallback = SuggestInstallDirectory(_defaults);
        if (string.Equals(fallback, requested, StringComparison.OrdinalIgnoreCase)
            || !InstallPathAccess.IsWritable(fallback))
        {
            fallback = InstallPathAccess.UserInstallDirectory(InstallFolderName(_defaults));
        }

        if (!InstallPathAccess.IsWritable(fallback))
        {
            return requested;
        }

        // No elevation prompt here: startup is not an explicit user action. The administrator grant that
        // would make the shared folder work is offered when the player saves settings.
        DetailText =
            $"Cannot write to {requested}, so the client will install under {fallback} instead. " +
            "Click Save, then Install.";
        SelectedTabIndex = SettingsTab;
        StatusText = "Install folder isn't writable.";

        if (persist)
        {
            _state.InstallDirectory = fallback;
            _stateStore.Save(_state);
        }
    ...
    private static string AccessDeniedMessage(UnauthorizedAccessException ex) =>
        $"{ex.Message} Pick a different install folder in Settings and click Save; approve the " +
        "administrator prompt to let every account on this PC use it.";

    /// <summary>
    /// Where the client should live: the distributor's folder when it set one, otherwise a machine-wide
    /// folder under ProgramData so every account on the PC plays the same install. Falls back to a
    /// per-user folder only when the shared one cannot be written, which happens when another account
    /// created it and the all-users grant has not been made yet.
    /// </summary>
    private static string SuggestInstallDirectory(LauncherDefaults defaults)
    {
        ...
        {
            var explicitDir = defaults.DefaultInstallDirectory.Trim();
            if (InstallPathAccess.IsWritable(explicitDir))
            {
                return explicitDir;
            }
        }

        var machineDir = InstallPathAccess.MachineInstallDirectory(InstallFolderName(defaults));
        return InstallPathAccess.IsWritable(machineDir)
            ? machineDir
            : InstallPathAccess.UserInstallDirectory(InstallFolderName(defaults));
    }

    private static string InstallFolderName(LauncherDefaults defaults) =>
        !string.IsNullOrWhiteSpace(defaults.AppName) ? defaults.AppName!
        : !string.IsNullOrWhiteSpace(defaults.DefaultInstallSubfolder) ? defaults.DefaultInstallSubfolder!
        : InstallPathAccess.DefaultFolderName;

    /// <summary>
    /// Opens the install folder to every account on the PC, prompting for administrator rights once per
    /// folder. Downloading is the only part that needs privilege, so the prompt happens here (on an
    /// explicit Save) rather than at startup, and the launcher goes back to running unelevated after it.
    ///
    /// When the grant cannot be made and the folder is not writable as-is, the launcher records that it
    /// must elevate on every start instead; the game is still started with the unelevated Explorer token
    /// so Wow.exe keeps resolving Data\ from the install folder.
    ///
    /// The grant walks every existing file, so over a full install it can take minutes and must stay off
    /// the UI thread.
    /// </summary>
    private async Task ShareInstallDirectoryWithAllUsersAsync(string installDir)
    {
        ...
        var alreadyShared = string.Equals(
            _state.SharedInstallDirectory ?? string.Empty,
            installDir,
            StringComparison.OrdinalIgnoreCase);
        if (alreadyShared && InstallPathAccess.IsWritable(installDir))
        {
            return;
        }

        var previousStatus = StatusText;
        StatusText = "Setting install folder permissions...";
        bool granted;
        try
        {
            granted = await InstallPathAccess.TryPrepareSharedDirectoryElevatedAsync(
                installDir, _defaults.DefaultInstallDirectory);
        }
        finally
        {
        ...
        if (granted)
        {
            _state.SharedInstallDirectory = installDir;
            ElevateOnStart = false;
            return;
        }

        _state.SharedInstallDirectory = null;

        if (InstallPathAccess.IsWritable(installDir))
        {
            ElevateOnStart = false;
            DetailText =
                $"{installDir} is set up for this Windows account only. To let every account on this PC " +
                "use the same install, click Save again and approve the administrator prompt.";
            return;
        }

        ElevateOnStart = true;
        DetailText =
            $"{installDir} needs administrator rights to write. The launcher will start as administrator " +
            "from now on; the game itself still starts normally.";
    }
...
""";
    ...
    private async Task SaveSettingsAsync()
    {
        ...
        try
        {
            ...
            var installDir = InstallDirectory.Trim();
            if (!InstallPathAccess.IsWritable(installDir))
            {
                var fallback = SuggestInstallDirectory(_defaults);
                if (!InstallPathAccess.IsWritable(fallback))
                {
                    StatusText = "Cannot use install folder.";
                    DetailText =
                        $"Access to {installDir} was denied. Choose a folder under your user profile " +
                        "(not Program Files).";
                    return;
                }
                ...
                installDir = fallback;
                DetailText =
                    $"Cannot write to a protected folder. Using {fallback} instead. Click Install on the Play tab.";
            }
            ...
            await ShareInstallDirectoryWithAllUsersAsync(installDir);

            _state.ServerUrl = normalizedServerUrl;
            _state.InstallDirectory = installDir;
    ...
    /// <summary>
    /// Warning shown under the progress bar for conditions that do not stop the update but leave the
    /// install in a state the player should know about.
    /// </summary>
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(HasSyncWarning))]
    private string _syncWarning = string.Empty;

    public bool HasSyncWarning => !string.IsNullOrWhiteSpace(SyncWarning);

    /// <summary>
    /// Tells the player when <see cref="SyncService.PlanRemovals"/> declined to delete files it believed
    /// the server had dropped. Silence there would look like a successful update that quietly left stale
    /// MPQs loading in the game.
    /// </summary>
    private void ReportRefusedRemovals(SyncPlan plan)
    {
        if (plan.RefusedRemovals <= 0)
        {
            SyncWarning = string.Empty;
            return;
        }

        SyncWarning =
            $"Kept {plan.RefusedRemovals} file(s) the server no longer lists, because removing that many " +
            "at once looked wrong. If the server was recently reset, use Verify files once it has been " +
            "restocked.";
        CrashLog.Write(
            new InvalidOperationException($"Refused {plan.RefusedRemovals} removals as implausible."),
            "SyncService.PlanRemovals");
    }
    ...
    private async Task CheckForUpdatesAsync(bool fullVerify, bool pruneUnknown = false)
    {
        ...
        try
        {
            ...
            _pendingBasePlan = await sync.PlanAsync(
                ...
                _state.InstallDirectory!,
                new PreviouslySyncedPaths(_state.LastManagedPaths, _state.LastBasePaths),
                effectiveFullVerify,
                progress,
                _cts.Token);

            ReportRefusedRemovals(_pendingBasePlan);
    ...
    private async Task UpdateAsync()
    {
        ...
        try
        {
            ...
            _state.LastBasePaths = _pendingBasePlan.BasePaths;
            _stateStore.Save(_state);
        ...
        catch (UnauthorizedAccessException ex)
        {
            ...
            DetailText = AccessDeniedMessage(ex);
        }
        catch (Exception ex)
        {
            StatusText = "Update failed.";
    ...
    private async Task PlayAsync()
    {
        ...
        try
        {
            ...
            _profileContent.PromoteVirtualStoreOverlay(_state.InstallDirectory!);
