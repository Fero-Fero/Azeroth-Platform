...
public sealed class SyncService
{
    ...
    public async Task<SyncPlan> PlanAsync(
        ...
        PreviouslySyncedPaths previouslySynced,
        bool fullVerify,
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var file in manifest.Files)
        {
            ...
            {
                plan.BasePaths.Add(file.RelativePath);
            }
        ...
        var removals = PlanRemovals(plan, previouslySynced);
        plan.RefusedRemovals = removals.RefusedCount;

        foreach (var relativePath in removals.Paths)
        {
            string localPath;
            try
            {
                localPath = ToLocalPath(installDirectory, relativePath);
            }
            catch (InvalidOperationException)
            {
                // The state file is local and unsigned, so a junk entry is possible. Skipping it only
                // costs us one stale file; acting on it could delete outside the install directory.
                continue;
            }
    ...
    /// <summary>
    /// Works out which previously synced paths the manifest no longer lists. Base files are pruned
    /// alongside managed ones, so an MPQ dropped from the client actually disappears from installs
    /// instead of staying on disk and being loaded by the game forever.
    ///
    /// Refuses the whole set when it is implausibly large. The manifest is signature verified before
    /// planning, so this is defense in depth against a validly signed but wrong manifest — an operator
    /// purging the client volumes produces exactly that. Same guard and threshold as
    /// <see cref="PruneToServerManifest"/>.
    /// </summary>
    internal static RemovalPlan PlanRemovals(
        SyncPlan plan,
        PreviouslySyncedPaths previouslySynced,
        double maxDeleteFraction = 0.5)
    {
        ...
        current.UnionWith(plan.ManagedPaths.Select(NormalizeRelative));
        current.UnionWith(plan.BasePaths.Select(NormalizeRelative));

        var removals = previouslySynced.Managed
            .Concat(previouslySynced.Base)
            .Select(NormalizeRelative)
            .Where(path => !string.IsNullOrWhiteSpace(path) && !current.Contains(path))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToList();

        // An empty manifest never legitimately means "delete everything the player has"; it means the
        // server has no content right now, which is what an operator purge looks like mid-restock.
        if (current.Count == 0 && removals.Count > 0)
        {
            return RemovalPlan.Refuse(removals.Count);
        }

        if (removals.Count > absoluteFloor
            && (double)removals.Count / current.Count > maxDeleteFraction)
        {
            return RemovalPlan.Refuse(removals.Count);
        }

        return RemovalPlan.Remove(removals);
    }
