...
public sealed class ProfileContentService
{
    ...
    /// every start avoids stale MPQ indexing when overlay patches change. Also drops a VirtualStore
    /// Cache mirror so an elevated Wow.exe and a normal Wow.exe do not use different indexes.
    /// </summary>
    public void ClearClientCache(string install)
    {
        TryDeleteDirectory(CacheDir(install));
        var virtualStore = ResolveVirtualStoreMirror(install);
        if (virtualStore is not null)
        {
            TryDeleteDirectory(Path.Combine(virtualStore, "Cache"));
        }
    }

    /// <summary>
    /// When the install lives under a protected path (e.g. Program Files) and the launcher or a copy
    /// once ran without elevation, Windows may have written overlay MPQs into
    /// <c>%LOCALAPPDATA%\VirtualStore\...</c>. Unelevated Wow.exe reads those; an elevated Wow.exe
    /// reads the real <c>Data/</c> folder and misses <c>patch-W</c>. Move VirtualStore archives into
    /// the real install so both see the same files.
    /// </summary>
    public void PromoteVirtualStoreOverlay(string install)
    {
        ...
        var virtualStore = ResolveVirtualStoreMirror(install);
        if (virtualStore is null)
        {
            return;
        }

        var vsData = Path.Combine(virtualStore, "Data");
        if (!Directory.Exists(vsData))
        {
            return;
        }

        var liveData = DataDir(install);
        try
        {
            Directory.CreateDirectory(liveData);
        }
        catch
        {
            return;
        ...
        foreach (var source in Directory.EnumerateFiles(vsData, "*", SearchOption.TopDirectoryOnly))
        {
            if (!source.EndsWith(".mpq", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            try
            {
                var dest = Path.Combine(liveData, Path.GetFileName(source));
                if (!File.Exists(dest))
                {
                ...
                var sourceLen = new FileInfo(source).Length;
                var destLen = new FileInfo(dest).Length;
            ...
            {
                // Best-effort: Play still starts Wow with a correct working directory.
            }
        }
    }

    /// <summary>
    /// <c>C:\Program Files\Foo</c> → <c>%LOCALAPPDATA%\VirtualStore\Program Files\Foo</c>.
    /// </summary>
    internal static string? ResolveVirtualStoreMirror(string installDirectory)
    {
        if (string.IsNullOrWhiteSpace(installDirectory) || !Path.IsPathRooted(installDirectory))
        {
            return null;
        }
        ...
        var relative = installDirectory[root.Length..].TrimStart(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
    ...
    public void Activate(string install, LauncherProfile profile, LauncherState state)
    {
        ...
        PromoteVirtualStoreOverlay(install);
