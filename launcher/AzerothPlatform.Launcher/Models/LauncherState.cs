...
public sealed class LauncherState
{
    ...
    /// <summary>
    /// Base file paths from the last sync, used to prune base files removed server-side. Empty for an
    /// install last synced by a launcher that did not record them; it fills in on the next sync, and
    /// until then base pruning simply has nothing to act on.
    /// </summary>
    public List<string> LastBasePaths { get; set; } = new();

    /// <summary>
    /// The install directory that has already been opened to all users by an elevated grant. Recorded so
    /// the UAC prompt appears once per folder rather than on every install.
    /// </summary>
    public string? SharedInstallDirectory { get; set; }

    /// <summary>
    /// Set when the shared-folder grant could not be made (declined, or not an administrator anywhere on
    /// this machine) and the install folder is otherwise unwritable. The launcher then elevates itself on
    /// every start so downloads still succeed; the game is still started unelevated.
    /// </summary>
    public bool ElevateOnStart { get; set; }
