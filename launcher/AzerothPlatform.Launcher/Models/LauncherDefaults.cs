...
public sealed class LauncherDefaults
{
    ...
    /// <c>C:/Program Files/{AppName}</c> and fetches multi-profile data from the backend.
    /// </summary>
    ...
    /// replicated registry, and verifies client manifests against this baked key (manifest trust no
    /// longer depends on a manager TLS channel now that config arrives over the stack's plain HTTP).
    /// </summary>
    public string? SigningPublicKey { get; set; }

    /// <summary>
    /// Suggested install folder name. Placed under the program-files folder
    /// (<c>C:\Program Files</c> on Windows). Defaults to "Azeroth Platform".
    /// </summary>
